...
import 'package:estore_app/enums/member_card_no_type.dart';
...
class MemberUtils {
  ...
  /// 查询会员信息
  /// 返回
  Future<Tuple4<bool, String, MemberCardNoType, Member>> httpQueryMemberInfo(String inputValue, {MemberCardNoType type = MemberCardNoType.None}) async {
    bool result = false;
    String msg = "无法识别该内容，请确认是否正确";
    Member member;
    try {
      type = MemberCardNoType.judgeCardWay(Global.instance.authc.tenantId, inputValue);
    ...
    return Tuple4<bool, String, MemberCardNoType, Member>(result, msg, type, member);
  }

  //查询会员充值方案
  Future<Tuple4<bool, int, String, MemberCardRechargeScheme>> httpMemberCardRechargeSchemeQuery(String cardNo) async {
    bool result = false;
    int code = -1;
    ...
    return Tuple4<bool, int, String, MemberCardRechargeScheme>(result, code, msg, scheme);
  }

  Future<Tuple3<bool, String, MemberTradePayResponse>> httpMemberConsume(String tradeNo, String memberId, String mobile, String cardNo, int isNoPwd, String passwd, int totalAmount, int pointValue) async {
    bool result = false;
    ...
    try {
      ...
      FLogger.info("会员<ID:$memberId 手机号:$mobile>卡<$cardNo>发起支付金额<$totalAmount>积分<$pointValue>");
      var response = await HttpUtils.instance.post(api, api.url, params: parameters);
  ...
  Future<Tuple3<bool, String, MemberTradeRefundResponse>> httpMemberRefund(String refundTradeNo, String tradeNo, String reason, int totalAmount, int pointValue, String marketRefundNo) async {
    bool result = false;
    ...
    try {
      ...
      var response = await HttpUtils.instance.post(api, api.url, params: parameters);
      FLogger.info("订单[$tradeNo]发起退款[$totalAmount]退积分[￥pointValue]返回结果：$response");
  ...
  //查询会员优惠券
  Future<Tuple3<bool, String, List<MemberElecCoupon>>> httpElecCouponList(String memberId, String couponNo, int status, int pageNumber, int pageSize) async {
    bool result = false;
  ...
  Future<Tuple3<bool, String, Map<String, dynamic>>> httpMemberConsumeCheck(MemberCardPayEntity pay) async {
    bool result = false;
    ...
    try {
      ...
      FLogger.info("会员<ID:${pay.memberId} 手机号:${pay.mobile}>卡<${pay.cardNo}>发起支付校验金额<${pay.totalAmount}>积分<${pay.pointValue}>");
      var response = await HttpUtils.instance.post(api, api.url, params: parameters);
      FLogger.info("会员<ID:${pay.memberId} 手机号:${pay.mobile}>卡<${pay.cardNo}>支付校验返回结果：$response");
  ...
  ///优惠券核销
  Future<Tuple3<bool, String, Map<String, dynamic>>> httpElecCouponChargeOff(String orderId, String orderNo, String memberId, String cardNo, List<ElecCouponCheckDetail> details) async {
    bool result = false;
    String msg = "优惠券核销异常";
    Map<String, dynamic> dataMap;
  ...
  Future<Tuple3<bool, String, Map<String, dynamic>>> orderElecCouponChargeOff(OrderObject order) async {
    ...
    //折扣券
    var promotionCouponList = order.promotions.where((x) => x.promotionType == PromotionType.Coupon && StringUtils.isNotBlank(x.couponId));
    ...
    //代金券
    var payCouponList = order.pays.where((x) => x.no == Constants.PAYMODE_CODE_COUPON && StringUtils.isNotBlank(x.couponId));
    ...
    return httpElecCouponChargeOff(order.id, order.tradeNo, memberId, cardNo, details);
  }

  Future<Tuple3<bool, String, Map<String, dynamic>>> sendOrderTicket(OrderObject orderObject) async {
  ...
  /// 获取优惠券可用订单明细
  List<OrderItem> getFineCouponItem(OrderObject orderObject, MemberElecCoupon coupon) {
    List<OrderItem> fineOrderItem;
    if (orderObject != null && orderObject.items.length > 0) {
      //待分摊商品
      fineOrderItem = orderObject.items.where((x) => x.rowType != OrderItemRowType.Detail && x.rowType != OrderItemRowType.SuitDetail && x.totalReceivableRemoveCouponLeastCost > 0 && (coupon.discountType == 1 ? true : x.foreDiscount == 1)).toList();
        ...
        case 1:
          {
            ...
            if (coupon.exgoodsList != null && coupon.exgoodsList.length > 0) {
              //排除黑名单中的商品
              fineOrderItem = fineOrderItem.where((x) => !coupon.exgoodsList.any((y) => y.specId == x.specId)).toList();
            }
          }
          break;
        case 2:
          {
            ...
            if (null != coupon.goodsList && coupon.goodsList.length > 0) {
              fineOrderItem = fineOrderItem.where((x) => coupon.goodsList.any((y) => y.specId == x.specId)).toList();
            }
          }
          break;
        case 3:
          {
            ...
            if (null != couponCategorys && couponCategorys.length > 0) {
              //类别且排除黑名单商品的应收金额
              fineOrderItem = fineOrderItem.where((x) => couponCategorys.any((y) => y.categoryId == x.categoryId) && (coupon.exgoodsList == null || !coupon.exgoodsList.any((z) => z.specId == x.specId))).toList();
            }
          }
          break;
        case 4:
          {
            ...
            if (null != couponBrands && couponBrands.length > 0) {
              //类别且排除黑名单商品的应收金额
              fineOrderItem = fineOrderItem.where((x) => couponBrands.any((y) => y.brandId == x.brandId) && (coupon.exgoodsList == null || !coupon.exgoodsList.any((z) => z.specId == x.specId))).toList();
            }
          }
          break;
        default:
  ...
  /// 应用折扣券
  Tuple2<bool, String> applyDiscountCoupon(OrderObject orderObject, MemberElecCoupon coupon) {
    bool result = false;
    ...
    try {
      if (orderObject != null && orderObject.items.length > 0) {
        ...
        if (fineOrderItem.length > 0) {
          ...
          if (orderObject.promotions != null && orderObject.promotions.any((x) => x.promotionType == PromotionType.Coupon)) {
            //移除已用折扣券
            orderObject.promotions.removeWhere((x) => x.promotionType == PromotionType.Coupon);
            orderObject.items.forEach((x) {
              x.promotions.removeWhere((y) => y.promotionType == PromotionType.Coupon);
              //重算价格
          ...
          promotion = PromotionUtils.instance.newOrderPromotion(orderObject, promotionType: PromotionType.Coupon);
          ...
          promotion.discountRate = OrderUtils.instance.toRound(coupon.discountValue / 100);
          promotion.discountAmount = OrderUtils.instance.toRound(orderObject.amount * promotion.discountRate);
          promotion.enabled = 0;
  ...
  Tuple2<bool, String> applyMoneyCoupon(OrderObject orderObject, OrderPay pay) {
    ...
    try {
      if (orderObject != null && orderObject.items.length > 0) {
        //排除当前支付方式的剩余支付金额
        var orderBalancePay = orderObject.paidAmount - orderObject.pays.where((x) => x.id != pay.id).map((e) => e.paidAmount).fold(0, (prev, paidAmount) => prev + paidAmount);
        ...
        if (fineOrderItem.length > 0) {
          //参与商品净额（商品应收 - 券占金额）
          var totalAmount = fineOrderItem.map((e) => e.totalReceivableRemoveCouponLeastCost).fold(0, (prev, leastCost) => prev + leastCost);

          //面值分摊
          var discountAmountSum = pay.faceAmount;
          //分摊最低消费金额(考虑满足0元就可以使用的券，取面值、最低金额最大值)
          var discountLeastCostSum = pay.couponLeastCost > pay.faceAmount ? pay.couponLeastCost : pay.faceAmount;
          ...
          for (var item in fineOrderItem) {
            ...
            if (i >= fineOrderItem.length) {
              ...
              if (totalAmount != 0) {
                //这里使用去券金额，表示这个还能够用券
                var rate = item.totalReceivableRemoveCouponLeastCost / totalAmount;

                share = OrderUtils.instance.toRound(discountAmountSum * rate);

                shareLeastCost = OrderUtils.instance.toRound(discountLeastCostSum * rate);
              }
            }
  ...
  ///检测优惠券的可用情况
  void checkCouponEffect(List<MemberElecCoupon> couponList, List<MemberElecCoupon> couponSelected, OrderObject orderObject, {bool topSelect = false}) {
    try {
      //没有优惠券，直接返回
      ...
      //只校验未选中的
      var noSelectedCoupons = couponList.where((x) => !coupons.contains(x.couponNo));
      noSelectedCoupons.forEach((coupon) {
        ...
        if (isGo && coupon.dayLimitQuantity != 0 && coupon.usedNum + count >= coupon.dayLimitQuantity) {
          isGo = false;
        ...
        if (isGo && coupon.useTimeFlag == 1) {
          ...
          if (null != couponTimes && couponTimes.length > 0) {
            ...
            for (ElecCouponUseTime time in couponTimes) {
              time.currentTime = DateTimeUtils.formatDate(now, format: "yyyy-MM-dd HH:mm:ss");
              if (time.checkTime) {
                timeCheck = true;
                break;
              }
        ...
        if (isGo && coupon.consumeStoreFlag == 1) {
          var storeCheck = coupon.consumeStoreList.any((x) => x.storeId == Global.instance.authc.storeId);
          if (!storeCheck) {
            isGo = false;
        ...
        if (isGo && null != orderObject && orderObject.items.length > 0) {
          //待分摊商品
          var fineOrderItem = orderObject.items.where((x) => x.rowType != OrderItemRowType.Detail && x.rowType != OrderItemRowType.SuitDetail && x.totalReceivableRemoveCouponLeastCost > 0 && (coupon.discountType == 1 ? true : x.foreDiscount == 1));
          switch (coupon.fitRange) {
            case 1: // 全场通用
              {
                //券后应收金额
                ...
                if (coupon.exgoodsList != null && coupon.exgoodsList.length > 0) {
                  var currentFineOrderItems = fineOrderItem.where((x) => !coupon.exgoodsList.any((y) => y.specId == x.specId));
                  if (currentFineOrderItems != null && currentFineOrderItems.length > 0) {
                    //排除黑名单中的商品的剩余应收金额
                    receivableAmount = currentFineOrderItems.map((e) => e.totalReceivableRemoveCouponLeastCost).fold(0, (prev, leastCost) => prev + leastCost);
                  } else {
                    isGo = false;
                    reason.write("没有可用商品;");
                  }
                } else {
                  receivableAmount = fineOrderItem.map((e) => e.totalReceivableRemoveCouponLeastCost).fold(0, (prev, leastCost) => prev + leastCost);
                }
            ...
            case 2: // 部分商品
              {
                if (null != coupon.goodsList && coupon.goodsList.length > 0) {
                  var currentFineOrderItems = fineOrderItem.where((x) => coupon.goodsList.any((y) => y.specId == x.specId));
                  if (currentFineOrderItems != null && currentFineOrderItems.length > 0) {
                    var receivableAmount = currentFineOrderItems.map((e) => e.totalReceivableRemoveCouponLeastCost).fold(0, (prev, leastCost) => prev + leastCost);
                    ...
                    if (isGo && receivableAmount < coupon.leastCost) {
                      isGo = false;
                      reason.write("部分商品未满${coupon.leastCost}元;"); //,参与金额[$receivableAmount]元
                    }
                  } else {
                    isGo = false;
                    reason.write("没有可用商品;");
            ...
            case 3: // 部分类别
              {
                List<ElecCouponCategory> couponCategorys = coupon.categoryList;
                if (null != couponCategorys && couponCategorys.length > 0) {
                  var currentFineOrderItems = fineOrderItem.where((x) => couponCategorys.any((y) => y.categoryId == x.categoryId) && (coupon.exgoodsList == null || !coupon.exgoodsList.any((z) => z.specId == x.specId)));
                  if (currentFineOrderItems != null && currentFineOrderItems.length > 0) {
                    //类别且排除黑名单商品的应收金额
                    var receivableAmount = currentFineOrderItems.map((e) => e.totalReceivableRemoveCouponLeastCost).fold(0, (prev, leastCost) => prev + leastCost);
                    //剩余券后应收为零
                    ...
                    if (isGo && receivableAmount < coupon.leastCost) {
                      isGo = false;
                      reason.write("部分品类消费未满${coupon.leastCost}元;"); //,当前参与金额[$receivableAmount]
                    }
                  } else {
                    isGo = false;
            ...
            case 4: // 部分品牌
              {
                List<ElecCouponBrand> couponBrands = coupon.brandList;
                if (null != couponBrands && couponBrands.length > 0) {
                  var currentFineOrderItems = fineOrderItem.where((x) => couponBrands.any((y) => y.brandId == x.brandId) && (coupon.exgoodsList == null || !coupon.exgoodsList.any((z) => z.specId == x.specId)));
                  if (currentFineOrderItems != null && currentFineOrderItems.length > 0) {
                    //类别且排除黑名单商品的应收金额
                    var receivableAmount = currentFineOrderItems.map((e) => e.totalReceivableRemoveCouponLeastCost).fold(0, (prev, leastCost) => prev + leastCost);
                    //剩余券后应收为零
                    ...
                    if (isGo && receivableAmount < coupon.leastCost) {
                      isGo = false;
                      reason.write("部分品牌消费未满[${coupon.leastCost}]元;"); //,当前参与金额[$receivableAmount]
                    }
                  } else {
                    isGo = false;
      ...
      if (couponSelected.length > 0) {
        ...
        //未选择的券中获取可用的券
        var enableCouponList = noSelectedCoupons.where((x) => x.enable).toList();
        ...
        //没有被选中 && 启用 && 折扣券 统统否决掉
        var discountCoupons = noSelectedCoupons.where((x) => x.enable && x.discountType == 2);
  ...
  //处理折扣券
  Future<void> processDiscountCoupon(List<MemberElecCoupon> couponList, List<MemberElecCoupon> couponSelected, OrderObject orderObject) async {
    //获取选中的全部折扣券
    List<MemberElecCoupon> allDiscountCoupons = couponSelected.where((x) => x.discountType == 2).toList();
    print("已经选择的折扣券数量:${allDiscountCoupons.length}");
    if (allDiscountCoupons.length > 0) {
      //校验是否已使用
      allDiscountCoupons.forEach((coupon) {
        var couponPromotion = orderObject.promotions.firstWhere((x) => coupon.couponNo == x.couponNo, orElse: () => null);
      ...
      //删除主单和明细的折扣券优惠
      orderObject.promotions.removeWhere((x) => x.promotionType == PromotionType.Coupon);
      orderObject.items.forEach((x) => x.promotions.removeWhere((y) => y.promotionType == PromotionType.Coupon));
      //重算订单明细，订单汇总
      updateAllRow(orderObject);
    }
  }
  ...
  //处理代金券,代金券记入支付方式
  Future<void> processCashCoupon(List<MemberElecCoupon> couponList, List<MemberElecCoupon> couponSelected, OrderObject orderObject) async {
    //删除取消的优惠券
    if (couponSelected.length > 0) {
      var couponPayList = orderObject.pays.where((x) => x.no == Constants.PAYMODE_CODE_COUPON && StringUtils.isNotBlank(x.couponNo));
      if (couponPayList.length > 0) {
        //找到已删除的优惠券，删除支付方式分摊
        var delPayList = couponPayList.where((x) => !couponSelected.any((y) => y.couponNo == x.couponNo));
        if (delPayList != null && delPayList.length > 0) {
          orderObject.pays.removeWhere((x) => delPayList.any((y) => y.id == x.id));
          //删除支付方式分摊
          orderObject.items.forEach((x) => x.itemPays.removeWhere((y) => delPayList.any((z) => z.id == y.payId)));
          //重算订单明细，订单汇总
          updateAllRow(orderObject);
        }
      }
    ...
    if (couponSelected != null && couponSelected.length > 0) {
      for (var coupon in couponSelected) {
        ...
        if (orderObject.pays.any((x) => x.no == Constants.PAYMODE_CODE_COUPON && x.couponNo == coupon.couponNo)) {
          //该代金券支付方式已存在
          continue;
        }
        ...
        if (coupon.discountType == 1) {
          //代金券
          var couponPay = orderObject.pays.firstWhere((x) => x.couponNo == coupon.couponNo, orElse: () => null);
          if (couponPay == null) {
            //新增
            var payMode = await OrderUtils.instance.getPayMode(Constants.PAYMODE_CODE_COUPON);
            if (payMode == null) {
              return;
            }
      ...
      //清空卡券支付方式
      orderObject.pays.removeWhere((x) => x.no == Constants.PAYMODE_CODE_COUPON && StringUtils.isNotBlank(x.couponId));
    }
  }
}
