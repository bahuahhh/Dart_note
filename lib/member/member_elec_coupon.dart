...
class MemberElecCoupon extends Equatable {
  ...
  String get effectiveTime {
    ...
    try {
      var begin = DateTimeUtils.getDateTime(beginEffectiveTime);
      var end = DateTimeUtils.getDateTime(endEffectiveTime);
  ...
  factory MemberElecCoupon.fromJson(Map<String, dynamic> json) {
    return MemberElecCoupon()
      ...
      ..categoryList = json["categoryList"] != null ? List<ElecCouponCategory>.from(json["categoryList"].map((x) => ElecCouponCategory.fromJson(x))) : <ElecCouponCategory>[]
      ..brandList = json["brandList"] != null ? List<ElecCouponBrand>.from(json["brandList"].map((x) => ElecCouponBrand.fromJson(x))) : <ElecCouponBrand>[]
      ..goodsList = json["goodsList"] != null ? List<ElecCouponGoods>.from(json["goodsList"].map((x) => ElecCouponGoods.fromJson(x))) : <ElecCouponGoods>[]
      ..exgoodsList = json["exgoodsList"] != null ? List<ElecCouponGoods>.from(json["exgoodsList"].map((x) => ElecCouponGoods.fromJson(x))) : <ElecCouponGoods>[]
      ..consumeStoreList = json["consumeStoreList"] != null ? List<ElecCouponStore>.from(json["consumeStoreList"].map((x) => ElecCouponStore.fromJson(x))) : <ElecCouponStore>[]
      ..useTimeList = json["useTimeList"] != null ? List<ElecCouponUseTime>.from(json["useTimeList"].map((x) => ElecCouponUseTime.fromJson(x))) : <ElecCouponUseTime>[];
  }
  ...
  Map<String, dynamic> toJson() {
    ...
    if (this.consumeStoreList != null) {
      data['consumeStoreList'] = this.consumeStoreList.map((v) => v.toJson()).toList();
    }
  ...
  factory MemberElecCoupon.clone(MemberElecCoupon obj) {
    return MemberElecCoupon()
      ...
      ..categoryList = obj.categoryList.map((e) => ElecCouponCategory.clone(e)).toList()
      ..brandList = obj.brandList.map((e) => ElecCouponBrand.clone(e)).toList()
      ..goodsList = obj.goodsList.map((e) => ElecCouponGoods.clone(e)).toList()
      ..exgoodsList = obj.exgoodsList.map((e) => ElecCouponGoods.clone(e)).toList()
      ..consumeStoreList = obj.consumeStoreList.map((e) => ElecCouponStore.clone(e)).toList()
      ..useTimeList = obj.useTimeList.map((e) => ElecCouponUseTime.clone(e)).toList();
  }
...
class ElecCouponUseTime {
  ...
  bool get checkTime {
    ...
    try {
      var _time = DateTimeUtils.formatDate(_currentTime, format: "HH:mm");
      if (_time.compareTo(this.beginTime) < 0 || _time.compareTo(this.endTime) >= 0) {
        return false;
      }
    } catch (e, stack) {
      FlutterChain.printError(e, stack);
