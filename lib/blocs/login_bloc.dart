  ...
  Stream<LoginState> _mapWorkerLoginToState(WorkerLogin event) async* {
    try {
      ...
      if (offline) {
        ...
        response = await AuthzUtils.instance.databaseLogin(state.workerNo, state.password);
      } else {
        FLogger.info("收银员${state.workerNo}联机登录中....");
        response = await AuthzUtils.instance.httpLogin(state.workerNo, state.password);
      }
...
class LoginState extends Equatable {
  ...
  @override
  List<Object> get props => [this.workerNo, this.workerNoValid, this.password, this.passwordValid, this.status, this.message];
}
