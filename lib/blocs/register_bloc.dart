...
import 'package:estore_app/entity/pos_apis.dart';
import 'package:estore_app/entity/pos_authc.dart';
import 'package:estore_app/entity/registration_code.dart';
...
class RegisterBloc extends Bloc<RegisterEvent, RegisterState> {
  ...
  Stream<RegisterState> _mapSuccessfulToState() async* {
    try {
      ...
      Tuple3<bool, String, Authc> response; //= await this._registerRepository.httpRegistApi(state.registrationCode, state.authc);

      if (state.activeCode.startsWith("57110")) {
        response = await this._registerRepository.oldHttpRegistApi(state.registrationCode, state.authc);
      } else {
      ...
      if (response.item1) {
        ...
        Tuple2<bool, String> result; //= await this._registerRepository.saveRegister(state.registrationCode, Global.instance.authc);

        if (state.activeCode.startsWith("57110")) {
          result = await this._registerRepository.oldSaveRegister(state.registrationCode, Global.instance.authc);
        } else {
  ...
  Stream<RegisterState> _mapSubmittedToState() async* {
    ...
    try {
      Tuple4<bool, String, RegistrationCode, Authc> response; //= await this._registerRepository.httpActiveApi(state.activeCode);
      if (state.activeCode.startsWith("57110")) {
        response = await this._registerRepository.oldHttpActiveApi(state.activeCode);
      } else {
...
enum RegisterStatus { None, Initial, Loading, Confirm, Waiting, Success, Failure }
...
class RegisterState extends Equatable {
  ...
  @override
  List<Object> get props => [this.activeCode, this.activeCodeValid, this.status, this.message, this.registrationCode, this.authc];
}
...
class RegisterRepository {
  ///提交激活码到服务端
  Future<Tuple4<bool, String, RegistrationCode, Authc>> httpActiveApi(String activeCode) async {
    ...
    try {
      ...
      FLogger.debug("computerName=$computerName,macAddress=$macAddress,serialNumber=$serialNumber,cpuNumber=$cpuNumber,systemVersion=$systemVersion");
      ...
      var response = await HttpUtils.instance.post(api, api.open, params: parameters);
    ...
    return Tuple4<bool, String, RegistrationCode, Authc>(result, msg, registrationCode, authc);
  }

  Future<Tuple3<bool, String, Authc>> httpRegistApi(RegistrationCode registrationCode, Authc authc) async {
    bool result = false;
    String msg = "";
    try {
      OpenApi api = OpenApiUtils.instance.getOpenApi(ApiType.Business);
  ...
  Future<Tuple2<bool, String>> saveRegister(RegistrationCode registrationCode, Authc authc) async {
    bool result = false;
    String msg = "";
  ...
  ///提交激活码到服务端
  Future<Tuple4<bool, String, RegistrationCode, Authc>> oldHttpActiveApi(String activeCode) async {
    FLogger.debug("收到注册请求:$activeCode");
    ...
    try {
      ...
      FLogger.debug("computerName=$computerName,macAddress=$macAddress,serialNumber=$serialNumber,systemVersion=$systemVersion");
  ...
  Future<Tuple3<bool, String, Authc>> oldHttpRegistApi(RegistrationCode registrationCode, Authc authc) async {
    bool result = false;
    String msg = "";
    try {
      OpenApi api = OpenApiUtils.instance.getOpenApi(ApiType.Business);
      ...
      var data = {"posId": registrationCode.posId, "posNo": registrationCode.posNo, "appSign": Constants.APP_SIGN, "terminalType": Constants.TERMINAL_TYPE, "authCode": registrationCode.authCode, "compterName": authc.compterName, "serialNumber": authc.diskSerialNumber, "cpuNumber": authc.cpuSerialNumber, "macAddress": authc.macAddress};
  ...
  Future<Tuple2<bool, String>> oldSaveRegister(RegistrationCode registrationCode, Authc authc) async {
    bool result = false;
    String msg = "";
