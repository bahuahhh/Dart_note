...
class SysInitBloc extends Bloc<SysInitEvent, SysInitState> {
  ...
  Stream<SysInitState> _mapSuccessfulToState() async* {
    try {
      ...
      Tuple2<bool, String> result = await this._sysInitRepository.clearAllData();
  ...
  Stream<SysInitState> _mapSubmittedToState() async* {
    yield state.loading();
    try {
      var currentDate = DateTimeUtils.formatDate(DateTime.now(), format: "yyyyMMdd");
      var tenantCode = state.authc == null ? "" : state.authc.tenantId;

      var result = StringUtils.isBlank(state.safeCode) ? false : state.safeCode == "$currentDate$tenantCode";
      if (result) {
        yield state.confirm();
      } else {
        yield state.failure(
...
class SysInitState extends Equatable {
  ...
  @override
  List<Object> get props => [this.safeCode, this.safeCodeValid, this.status, this.message, this.authc];
}
