...
class HttpUtils {
  ...
  Dio _getDio(OpenApi api) {
    if (!_clients.containsKey(api.apiType)) {
      BaseOptions options = new BaseOptions(
        ...
        headers: {
          HttpHeaders.userAgentHeader: "Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/51.0.2704.106 Safari/537.36",
  ...
  Future<OpenResponse> post(OpenApi api, String url, {Map<String, dynamic> params, Options options}) async {
    final watch = Stopwatch()..start();
    try {
      var client = this._getDio(api);
      Response response = await client.post(url, data: params, options: options);
  ...
  Future<void> posMonitor({String terminalType = Constants.TERMINAL_TYPE}) async {
    ...
    try {
      ...
      var storeName = storeInfo != null ? storeInfo.name : Global.instance.authc.storeName;
    ...
    if (200 == status || 201 == status) {
      ...
      if (_code == 0) {
        ...
        if (_data is Map) {
          ...
          return OpenResponse(_code, _msg, List<Map<String, dynamic>>.from(_data));
