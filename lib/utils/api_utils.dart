...
import 'package:estore_app/entity/pos_urls.dart';
...
class OpenApiUtils {
  ...
  Future init() async {
    ...
    ///获取系统登记的全部可用访问地址
    var urlMaps = await database.rawQuery("select id,tenantId,apiType,protocol,url,contextPath,userDefined,enable,memo,ext1,ext2,ext3,createUser,createDate,modifyUser,modifyDate from pos_urls;");
    ...
    ///数据表中获取Map对象
    var apiMaps = await database.rawQuery("select id,tenantId,apiType,appKey,appSecret,locale,format,client,version,routing,memo,ext1,ext2,ext3,createUser,createDate,modifyUser,modifyDate from pos_apis;");
    apiMaps.forEach((map) {
      ///获取对象
      ...
      ///查找ApiType匹配的访问地址列表
      var servers = urls.where((obj) => Comparable.compare(obj.apiType, api.apiType) == 0).toList();
  ...
  Map<String, String> newParameters({OpenApi api}) {
    try {
      ...
      parameters["timestamp"] = DateTimeUtils.formatDate(DateTime.now(), format: "yyyy-MM-dd HH:mm:ss");
  ...
  String sign(OpenApi api, Map<String, String> parameters, List<String> ignoreParameters) {
    try {
