...
class FlutterChain {
  ...
  static printError(_error, _stack, {bool simple: true}) {
    ...
    if (errorStr.isNotEmpty) debugLog(errorStr, isShowTime: false, showLine: true);
  }
  ...
  static String _parseFlutterStack(Trace _trace) {
    ...
    _strs.forEach((_str) {
      if (!_str.contains("package:flutter/") && !_str.contains("dart:") && !_str.contains("package:flutter_stack_trace/")) {
        if (_str.isNotEmpty) {
...
void debugLog(Object obj, {bool isShowTime = true, bool showLine = false}) {
  ...
  assert(isDebug = true);
  if (isDebug) {
    String log = isShowTime ? "${DateTime.now()}:  ${obj.toString()}" : "${obj.toString()}";
    if (showLine) {
