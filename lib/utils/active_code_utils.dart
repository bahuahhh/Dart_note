...
import 'package:estore_app/blocs/register_bloc.dart';
...
class ActiveCodeUtils {
  ...
  String generateValidKey(Authc auth) {
    ...
    if (obj != null) {
      ...
      return StringUtils.reverse(Md5Utils.md5(sourceStr).substring(16)).toLowerCase();
    }
    return null;
  }
  ...
  /// 获取最新认证信息
  Future<Tuple4<bool, int, String, ActiveCodeEntity>> getActiveInfo(Authc auth) async {
    bool result = false;
    int code = -1;
  ...
  Tuple3<bool, bool, String> validActiveCode(DateTime now) {
    ...
    try {
      ...
      if (activeCodeObj != null) {
        ...
        //过期时间
        var expiryDate = DateTime.fromMillisecondsSinceEpoch(int.tryParse("${activeCodeObj.expiryDate}"));
        //剩余时间
        int remainDays = expiryDate.difference(now).inDays;
        if (remainDays > 0 && remainDays <= activeCodeObj.remindDay) {
          result = true;
          notify = true;
          message = "您的账号将于${remainDays == 0 ? '今天' : '$remainDays天后'}${activeCodeObj.trialFlag == 1 ? '试用' : ''}到期，\n请提前安排续费";
        } else if (remainDays <= 0) {
          result = false;
          notify = true;
          message = "您的账号${activeCodeObj.trialFlag == 1 ? '试用' : ''}已到期，请续费，\n\n如已续费，请重新打开应用。";
        } else {
          result = true;
          notify = false;
          message = "正常";
