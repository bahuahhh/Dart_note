...
class AuthDialogPage extends StatefulWidget {
  ...
  AuthDialogPage(this.moduleKeyCode, {this.orderObject, this.permissionCode = Constants.PERMISSION_CODE, this.discountRate = double.maxFinite, this.freeAmount = double.maxFinite, this.onAction, this.onClose});

  @override
  State<StatefulWidget> createState() => _AuthDialogPageState();
}

class _AuthDialogPageState extends State<AuthDialogPage> with SingleTickerProviderStateMixin {
  ///操作员工号
  ...
  Widget _buildContent() {
    return Container(
      ...
      child: Column(
        ...
        children: <Widget>[
          ConstrainedBox(
            constraints: BoxConstraints(maxHeight: Constants.getAdapterHeight(75), maxWidth: Constants.getAdapterWidth(600)),
  ...
  Future<void> login() async {
    try {
      ...
      var loginResult = await AuthzUtils.instance.databaseLogin(workerNo, passwd);

      if (loginResult.item1) {
        var permission = await AuthzUtils.instance.getPermission(loginResult.item3);
        var checkPermission = AuthzUtils.instance.checkPermission(widget.permissionCode, permission, widget.discountRate, widget.freeAmount);
        ...
        children: <Widget>[
          Expanded(
            child: Container(
              ...
              child: Text("授权[${widget.moduleKeyCode.name}]", style: TextStyles.getTextStyle(color: Constants.hexStringToColor("#FFFFFF"), fontSize: 32)),
            ),
          ),
          InkWell(
            onTap: () {
...
class AuthzUtils {
  ...
  Tuple2<bool, String> checkAuthz(BuildContext context, ModuleKeyCode moduleKeyCode, String permissionCode, OrderObject orderObject, PermissionAction action, {double expectDiscountRate = double.maxFinite, double expectFreeAmount = double.maxFinite}) {
    bool isAuthz = true;
    ...
    if (hasPermission(permissionCode)) {
      ...
      var authz = Tuple4(discountRate, freeAmount, permission, Global.instance.worker);
      var checkPermission = AuthzUtils.instance.checkPermission(permissionCode, authz, expectDiscountRate, expectFreeAmount);

      //有权限
      if (checkPermission.item1) {
        action(new Tuple4(discountRate, freeAmount, permission, Global.instance.worker));
      } else {
        YYDialog dialog;
        var onClose = () {
          dialog?.dismiss();
        ...
        dialog = _openAuthzDialog(context, authDialogPage, moduleKeyCode, permissionCode, orderObject, action, expectDiscountRate: expectDiscountRate, expectFreeAmount: expectFreeAmount);
      }
    } else {
      YYDialog dialog;
      var onClose = () {
  ...
  YYDialog _openAuthzDialog(BuildContext context, Widget child, ModuleKeyCode moduleKeyCode, String permissionCode, OrderObject orderObject, PermissionAction action, {double expectDiscountRate = double.maxFinite, double expectFreeAmount = double.maxFinite}) {
  ...
  Tuple2<bool, String> checkPermission(String permissionCode, Tuple4<double, double, List<String>, Worker> permission, double discountRate, double freeAmount) {
    bool result = true;
    String message = "授权成功";

    //功能权限不校验或者校验并拥有
    bool isPermission = (permissionCode == Constants.PERMISSION_CODE || permission.item3.contains(permissionCode));
    ...
    if (result && discountRate != double.maxFinite) {
      bool isDiscountRate = (0 == discountRate || permission.item1 <= discountRate);
      if (!isDiscountRate) {
        result = false;
  ...
  bool hasPermission(String moduleNo) {
    ...
    return Global.instance.worker != null && Global.instance.worker.permission != null && Global.instance.worker.permission.contains(moduleNo);
  }

  Future<Tuple3<bool, String, Worker>> databaseLogin(String workerNo, String passwd) async {
    bool result = false;
    String msg = "";
    Worker worker;
    try {
      ...
      if (worker != null) {
        var isCorrect = await FlutterBcrypt.verify(password: passwd, hash: worker.passwd);
        if (isCorrect) {
          result = true;
          msg = "登录认证通过";
        } else {
  ...
  Future<Tuple3<bool, String, Worker>> httpLogin(String workerNo, String passwd) async {
    bool result = false;
    String msg = "";
    Worker worker;
    try {
  ...
  Future<Tuple4<double, double, List<String>, Worker>> getPermission(Worker worker) async {
    //最大折扣
    ...
    try {
      if (worker != null) {
        String sql = "select * from pos_worker_role where userId = '${worker.id}';";
        var database = await SqlUtils.instance.open();
    ...
    print("收银员<${worker.no}>,最大折扣<$maxDiscountRate>,最大免单<$maxFreeAmount>,权限<${permission.toString()}>");
