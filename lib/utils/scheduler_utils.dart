    ...
    this.setInterval((time) async {
      ...
      if (waitingUpload != null && waitingUpload.length > 0) {
        ...
        for (var order in waitingUpload) {
          var orderObject = await OrderUtils.instance.builderOrderObject(order.id);
          ...
          if (buildResult.item1) {
            ...
            //uploadTime
            uploadResult["uploadTime"] = DateTimeUtils.formatDate(DateTime.now(), format: "yyyy-MM-dd HH:mm:ss");
            //modifyUser
            uploadResult["modifyUser"] = Global.instance.worker != null ? Global.instance.worker.no : Constants.DEFAULT_MODIFY_USER;
            //modifyDate
            uploadResult["modifyDate"] = DateTimeUtils.formatDate(DateTime.now(), format: "yyyy-MM-dd HH:mm:ss");
            //uploadCode
            uploadResult["uploadCode"] = resp.code;
            //uploadMessage
            uploadResult["uploadMessage"] = resp.msg;
            //uploadErrors
            uploadResult["uploadErrors"] = (orderObject.uploadErrors == null ? 0 : orderObject.uploadErrors);
            //serverId
            ...
            if (resp.success) {
              ...
              //错误次数+1
              uploadResult["uploadErrors"] = (orderObject.uploadErrors == null ? 0 : orderObject.uploadErrors) + 1;
            }
