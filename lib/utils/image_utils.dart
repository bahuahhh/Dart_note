...
class ImageUtils {
  ...
  static Future<void> downloadFiles(String url, {String dir = Constants.IMAGE_PATH}) async {
    ...
    if (!isExists) {
      ...
      if (response.statusCode == 200) {
        ...
        _storeImageInLocalStorage(dir, extension: extension, bytesFile: response.bodyBytes, fileNameUrl: url);
      } else {
        print("failure");
      }
    } else {
  ...
  static void _storeImageInLocalStorage(String dir, {File file, List<int> bytesFile, String extension, String fileNameUrl}) async {
    //
    if (file != null) {
      try {
        ...
        //save image
        File("$dir/${getNameOnly(file.path)}")..writeAsBytesSync(img.encodeJpg(image, quality: 95));
      ...
      File("$dir/${getNameOnly(fileNameUrl)}")..writeAsBytesSync(img.encodeJpg(image, quality: 95));
