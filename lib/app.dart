...
import 'package:estore_app/utils/permission_utils.dart';
import 'package:estore_app/utils/scheduler_utils.dart';
...
class _MyAppState extends State<MyApp> with WidgetsBindingObserver {
  ...
  Widget build(BuildContext context) {
    return MultiBlocProvider(
      ...
      child: BlocBuilder<AppBloc, AppState>(builder: (BuildContext context, AppState state) {
        return MaterialApp(
          ...
          localeListResolutionCallback: (deviceLocale, supportedLocales) {
            ...
            if (deviceLocale != null && deviceLocale.length > 0) {
              ...
              //将设备默认语言改造为程序支持的规则
              var adapterLocale = Locale.fromSubtags(languageCode: defaultLocale.languageCode, countryCode: defaultLocale.countryCode);
...
class _PermissionWidgetState extends State<PermissionWidget> {
  ...
  Widget build(BuildContext context) {
    //初始化屏幕适配器
    ScreenUtils.init(context, width: Constants.SCREEN_WIDTH, height: Constants.SCREEN_HEIGHT, allowFontScaling: true);
  ...
  ///获取存储权限
  Widget _requiredPermissionUi(BuildContext context) => FutureBuilder<List<SystemPermission>>(
      future: PermissionUtils.requiredPermission(context),
      builder: (BuildContext context, AsyncSnapshot<List<SystemPermission>> snapshot) {
        if (snapshot.hasData) {
          //是否有未授权的权限
          var lists = snapshot.data.where((obj) => (obj.status == PermissionStatus.undetermined || obj.status == PermissionStatus.denied || obj.status == PermissionStatus.permanentlyDenied));
          //尚未请求权限或者没有权限
  ...
  Widget _showPermissionUi(List<SystemPermission> perms) => Container(
        ...
        decoration: BoxDecoration(
          ...
          image: DecorationImage(
            image: AssetImage(ImageUtils.getImgPath("download/background", format: "jpg")),
            fit: BoxFit.fill,
          ),
        ),
        child: Center(
