...
import 'package:estore_app/enums/order_table_action.dart';
import 'package:estore_app/enums/order_table_status.dart';
import 'package:estore_app/enums/order_upload_source.dart';
import 'package:estore_app/enums/post_way_type.dart';
...
class TableUtils {
  ...
  OrderTable builderOrderTable(StoreTable table, int people, {String memo = ""}) {
    ...
    entity.openTime = DateTimeUtils.formatDate(DateTime.now(), format: "yyyy-MM-dd HH:mm:ss");
  ...
  //开台
  Future<Tuple3<bool, String, OrderObject>> saveOrderObjectFromOrderTable(OrderTable orderTable, {bool onlineFlag = false}) async {
    bool result = false;
    String message = "";
    //生成新的退单对象
    ...
    try {
      ...
      orderTable.createDate = DateTimeUtils.formatDate(DateTime.now(), format: "yyyy-MM-dd HH:mm:ss");
      ...
      var finishDate = DateTimeUtils.formatDate(DateTime.now(), format: "yyyy-MM-dd HH:mm:ss");
      //重置订单完成时间
      ...
      //pos_order表的SQL语句
      var order = await OrderUtils.instance.buildOrderObjectSql(newOrderObject, finishDate);
      if (order.item1) {
        queues.add(order.item3);
      }

      //pos_order_table表的SQL语句
      var table = await OrderUtils.instance.buildOrderTableSql(newOrderObject, finishDate);
      if (table.item1) {
  ...
  Future<Tuple2<bool, String>> clearOrderTable(OrderTable table) async {
    ...
    try {
      ...
      //清除pos_order_table表
      queues.add("update pos_order_table set tableStatus='${table.tableStatus}',tableAction='${table.tableAction}' where id = '${table.id}';");
  ...
  //清台,开了台尚未点单，直接删除数据
  Future<Tuple2<bool, String>> clearOrderTableFromOrderObject(OrderObject orderObject) async {
    bool result = false;
    String message = "";
    try {
      var queues = new Queue<String>();
      ...
      //清除pos_order_table表
      queues.add("delete from pos_order_table where orderId = '${orderObject.id}';");
  ...
  //转台
  Future<Tuple4<bool, String, OrderObject, StoreTable>> updateOrderObjectForTransferTable(StoreTable sourceTable, StoreTable targetTable) async {
    bool result = false;
    String message = "";
    OrderObject newOrderObject;
    try {
      ...
      //加载转台前的订单信息
      newOrderObject = await OrderUtils.instance.builderOrderObject(sourceTable.orderTable.orderId);

      //更新桌台信息中要转台的桌子，其他并台桌子是不能更新的
      if (newOrderObject.tables != null && newOrderObject.tables.any((x) => x.tableId == sourceTable.id)) {
        var table = newOrderObject.tables.lastWhere((x) => x.tableId == sourceTable.id);
        table.tableId = tableId;
        table.tableName = tableName;
        table.tableNo = tableNo;

        //更新pos_order_table表的SQL语句
        var orderTableSql = "update pos_order_table set tableId = '${table.tableId}',tableNo = '${table.tableNo}',tableName = '${table.tableName}' where id = '${table.id}';";
      ...
      //更新pos_order表的SQL语句
      var orderSql = "update pos_order set tableId = '${newOrderObject.tableId}',tableNo = '${newOrderObject.tableNo}',tableName = '${newOrderObject.tableName}' where id = '${newOrderObject.id}';";
      queues.add(orderSql);
      ...
      for (var x in newOrderObject.items) {
        ...
        var orderItemSql = "update pos_order_item set tableId = '${x.tableId}',tableNo = '${x.tableNo}',tableName = '${x.tableName}' where id = '${x.id}' and tableId = '${sourceTable.id}';";
        ...
        if (x.promotions != null && x.promotions.length > 0) {
          for (var y in x.promotions) {
            ...
            var orderItemPromotionSql = "update pos_order_item_promotion set tableId = '${y.tableId}',tableNo = '${y.tableNo}',tableName = '${y.tableName}' where id = '${y.id}' and tableId = '${sourceTable.id}';";
        ...
        if (x.flavors != null && x.flavors.length > 0) {
          for (var z in x.flavors) {
            ...
            var orderItemMakeSql = "update pos_order_item_make set tableId = '${z.tableId}',tableNo = '${z.tableNo}',tableName = '${z.tableName}' where id = '${z.id}' and tableId = '${sourceTable.id}';";
    ...
    return Tuple4<bool, String, OrderObject, StoreTable>(result, message, newOrderObject, targetTable);
  }

  //并台
  Future<Tuple3<bool, String, OrderObject>> updateOrderObjectForMergeTable(StoreTable masterTable, List<StoreTable> mergeList) async {
    bool result = false;
    String message = "";
    OrderObject newOrderObject;
    try {
      ...
      if (result) {
        ...
        //修改日期
        String modifyDate = DateTimeUtils.formatDate(DateTime.now(), format: "yyyy-MM-dd HH:mm:ss");
        ...
        //更新主单开台信息
        queues.add("update pos_order_table set tableAction='$tableAction',serialNo='$mergeNo',masterTable='${masterTable.orderTable.masterTable}',modifyUser='$modifyUser', modifyDate='$modifyDate' where id = '${masterTable.orderTable.id}';");
        ...
        for (var table in mergeList) {
          ...
          if (masterTable.orderTable.orderId != table.orderTable.orderId) {
            queues.add("delete from pos_order where id = '${table.orderTable.orderId}';");
          }

          ///被合并的桌台包含菜品，同时转移菜品
          if (table.orderTable.totalQuantity > 0) {
            //菜品信息转移到主单
            queues.add("update pos_order_item set orderId='$orderId',tradeNo='$tradeNo',modifyUser='$modifyUser', modifyDate='$modifyDate' where tableId = '${table.orderTable.tableId}' and orderId ='${table.orderTable.orderId}';");

            //菜品做法转移到主单
            queues.add("update pos_order_item_make set orderId='$orderId',tradeNo='$tradeNo',modifyUser='$modifyUser', modifyDate='$modifyDate' where tableId = '${table.orderTable.tableId}' and orderId ='${table.orderTable.orderId}';");

            //菜品优惠转移到主单  Promotion
            queues.add("update pos_order_item_promotion set orderId='$orderId',tradeNo='$tradeNo',modifyUser='$modifyUser', modifyDate='$modifyDate' where tableId = '${table.orderTable.tableId}' and orderId ='${table.orderTable.orderId}';");

            //每桌的整单优惠转移到主单
            queues.add("update pos_order_promotion set orderId='$orderId',tradeNo='$tradeNo',modifyUser='$modifyUser', modifyDate='$modifyDate' where tableId = '${table.orderTable.tableId}' and orderId ='${table.orderTable.orderId}';");
          }
        ...
        ///将待合并的桌台的消费数据合并到主桌
        //并台后主单的总数量
        var totalQuantity = masterTable.orderTable.totalQuantity + mergeList.map((p) => p.orderTable.totalQuantity).fold(0, (prev, quantity) => prev + quantity);
        //并台后主单的总金额
        var totalAmount = masterTable.orderTable.totalAmount + mergeList.map((p) => p.orderTable.totalAmount).fold(0, (prev, amount) => prev + amount);
        //并台后主单的总退数量
        var totalRefundQuantity = masterTable.orderTable.totalRefund + mergeList.map((p) => p.orderTable.totalRefund).fold(0, (prev, refund) => prev + refund);
        //并台后主单的总退金额
        var totalRefundAmount = masterTable.orderTable.totalRefundAmount + mergeList.map((p) => p.orderTable.totalRefundAmount).fold(0, (prev, refundAmount) => prev + refundAmount);
        //并台后主单的总优惠金额
        var discountAmount = masterTable.orderTable.discountAmount + mergeList.map((p) => p.orderTable.discountAmount).fold(0, (prev, discountAmount) => prev + discountAmount);
        //并台后主单的总应收金额
        var receivableAmount = masterTable.orderTable.receivableAmount + mergeList.map((p) => p.orderTable.receivableAmount).fold(0, (prev, receivableAmount) => prev + receivableAmount);
        //并台后主单的总赠送数量
        var totalGiveQuantity = masterTable.orderTable.totalGive + mergeList.map((p) => p.orderTable.totalGive).fold(0, (prev, totalGive) => prev + totalGive);
        //并台后主单的总赠送金额
        var totalGiveAmount = masterTable.orderTable.totalGiveAmount + mergeList.map((p) => p.orderTable.totalGiveAmount).fold(0, (prev, giveAmount) => prev + giveAmount);
        //并台后主单的总人数
        var totalPeople = masterTable.orderTable.people + mergeList.map((p) => p.orderTable.people).fold(0, (prev, people) => prev + people);
  ...
  //拆台
  Future<Tuple2<bool, String>> splitOrderTableFromOrderObject(OrderObject orderObject) async {
    bool result = false;
    String message = "";
    try {
      //复制新对象,处理主桌数据
      ...
      for (var orderTable in orderObject.tables) {
        if (orderTable.masterTable == 1) {
          ...
          orderTable.modifyDate = DateTimeUtils.formatDate(DateTime.now(), format: "yyyy-MM-dd HH:mm:ss");
          ...
          masterOrderObject.modifyDate = DateTimeUtils.formatDate(DateTime.now(), format: "yyyy-MM-dd HH:mm:ss");

          //对主桌重算订单
          OrderUtils.instance.calculateOrderObject(masterOrderObject);

          //更新主桌订单信息
          queues.add("update pos_order set tableNo='${masterOrderObject.tableNo}',tableName='${masterOrderObject.tableName}',modifyUser='${masterOrderObject.modifyUser}', modifyDate='${masterOrderObject.modifyDate}' where id = '${masterOrderObject.id}';");
          //更新主桌开台信息
          queues.add(
        ...
        if (ticketNoResult.item1) {
          ...
          //修改item订单归属
          var items = orderObject.items.where((x) => x.tableId == orderTable.tableId).toList();
          ...
          for (var x in items) {
            ...
            //变更桌台商品的订单归属
            queues.add("update pos_order_item set orderId='${x.orderId}',tradeNo='${x.tradeNo}' where id = '${x.id}';");
            ...
            for (var f in x.flavors) {
              ...
              //变更桌台商品的订单归属
              queues.add("update pos_order_item_make set orderId='${f.orderId}',tradeNo='${f.tradeNo}' where id = '${f.id}';");
            }
            //单品促销
            for (var d in x.promotions) {
              ...
              //变更桌台商品的订单归属
              queues.add("update pos_order_item_promotion set orderId='${d.orderId}',tradeNo='${d.tradeNo}' where id = '${d.id}';");
            }
          }
          ...
          var orderSql = await OrderUtils.instance.buildOrderObjectSql(newOrderObject, finishDate);
          if (orderSql.item1) {
  ...
  Future<Tuple3<bool, String, List<OrderItem>>> tryOrder(OrderObject orderObject, List<OrderTable> tables, {bool goBack = false}) async {
    bool result = true;
    ...
    try {
      ...
      //订单中已经下单的商品
      var orderList = orderObject.items.where((x) => x.orderRowStatus == OrderRowStatus.Order).toList();
      print("已经下单的商品数量：${orderList.length}");
      //订单中新增的商品
      var newOrderList = orderObject.items.where((x) => x.orderRowStatus == OrderRowStatus.New).toList();
      ...
      for (var table in tables) {
        ...
        //订单中等待下单的商品
        var waitOrderList = orderObject.items.where((x) => x.tableId == table.tableId && x.orderRowStatus == OrderRowStatus.Save).toList();
        ...
        if (newOrderList.length > 0) {
          ...
          for (var item in newOrderList) {
            ...
            //不是返回操作，设置为下单状态
            item.orderRowStatus = goBack ? OrderRowStatus.Save : OrderRowStatus.Order;
            //并台,多桌台情况下，重置批号，确保统一批号添加的
            if (tables.length > 1) {
              //如果订单中有相同规格ID的商品,标识后面附加相同规格的数量合计
              var specCount = orderObject.items.count((x) => x.specId == item.specId);
              specNo++;
              item.tableBatchTag = "$tableBatchTag${specCount > 0 ? '_${specCount + specNo}' : ''}";
          ...
          var orderItemSql = await OrderUtils.instance.buildOrderItemSql(orderObject, orderObject.finishDate, newItemList: newOrderList);
        ...
        if (!goBack && waitOrderList.length > 0) {
          for (var item in waitOrderList) {
            ...
            queues.add("update pos_order_item set orderRowStatus = '${item.orderRowStatus.value}' where id = '${item.id}';");
          }
        }
        ...
        if (goBack) {
          placeOrders = orderList.where((x) => x.tableId == table.tableId).map((e) => e.quantity).fold(0, (prev, quantity) => prev + quantity);
        } else {
          placeOrders = orderObject.items.where((x) => x.tableId == table.tableId).map((e) => e.quantity).fold(0, (prev, quantity) => prev + quantity);
        }
  ...
  ///购物车变更通知
  Future<Tuple2<bool, String>> changeShopCart(OrderObject orderObject, OrderTable table, OrderItem item) async {
    bool result = false;
    String message = "";
  ...
  ///购物车变更通知
  Future<Tuple2<bool, String>> notifyShopCart(OrderObject orderObject, OrderTable table) async {
    bool result = false;
    String message = "";
  ...
  //删除单品
  Future<Tuple2<bool, String>> deleteOrderItem(OrderObject orderObject, OrderItem orderItem, List<OrderTable> tables) async {
    bool result = false;
    String message = "";
    try {
      var queues = new Queue<String>();
      ...
      for (var table in tables) {
        ...
        var newOrderList =
            orderObject.items.where((x) => x.tableId == tableId && x.specId == specId && x.quantity == quantity && x.tableBatchTag == tableBatchTag && (x.orderRowStatus == OrderRowStatus.New || x.orderRowStatus == OrderRowStatus.Save)).toList();
      ...
      for (var item in newItems) {
        //清除pos_order_item表
        queues.add("delete from pos_order_item where orderId='${newOrderObject.id}' and id='${item.id}';");
        ...
        if (item.flavors.length > 0) {
          queues.add("delete from pos_order_item_make where orderId='${newOrderObject.id}' and itemId='${item.id}';");
          //删除订单明细对应的做法
          newOrderObject.items.map((e) {
            e.flavors.removeWhere((f) => f.orderId == newOrderObject.id && f.itemId == item.id);
          });
        }
        //清除pos_order_item_promotion表
        if (item.promotions.length > 0) {
          queues.add("delete from pos_order_item_promotion where orderId='${newOrderObject.id}' and itemId='${item.id}';");
          //删除订单明细对应的优惠
          newOrderObject.items.map((e) {
            e.promotions.removeWhere((p) => p.orderId == newOrderObject.id && p.itemId == item.id);
          });
        }
      }
      ...
      for (var table in tables) {
        ...
        //已经下单的商品数量
        table.placeOrders = newOrderObject.items.where((x) => x.tableId == table.tableId && x.orderRowStatus == OrderRowStatus.Order).map((e) => e.quantity).fold(0, (prev, quantity) => prev + quantity);

        //人均
        double perCapitaAmount = table.people > 0 ? OrderUtils.instance.toRound(table.totalAmount / table.people) : 0;
        //优惠率
        double discountRate = table.totalAmount > 0 ? OrderUtils.instance.toRound(table.discountAmount / table.totalAmount) : 0;
        //保存影响桌台汇总数据
        String orderTableSql = sprintf(
  ...
  //修改数量
  Future<Tuple2<bool, String>> changeQuantity(OrderObject orderObject, OrderItem orderItem, List<OrderTable> tables, double inputValue) async {
    bool result = false;
    String message = "";
    try {
      var queues = new Queue<String>();
      ...
      for (var table in tables) {
        ...
        //已经下单的商品数量
        table.placeOrders = orderObject.items.where((x) => x.tableId == table.tableId && x.orderRowStatus == OrderRowStatus.Order).map((e) => e.quantity).fold(0, (prev, quantity) => prev + quantity);
  ...
  //赠送
  Future<Tuple2<bool, String>> changeGift(OrderObject orderObject, OrderItem orderItem, List<OrderTable> tables, double giftQuantity, String giftReason, {bool cancelGift = false}) async {
    bool result = false;
    String message = "";
    try {
      var queues = new Queue<String>();
      ...
      for (var table in tables) {
        ...
        ///获取桌台的未下单商品的清单，桌台、规格、数量、批量添加标识、修改数量的商品ID
        var newOrderList = orderObject.items.where((x) => x.tableId == tableId && x.specId == specId && x.quantity == quantity && x.tableBatchTag == tableBatchTag).toList();

        for (var orderItem in newOrderList) {
          //复制优惠计算前已经有的优惠活动，主要处理新增、修改、删除SQL语句
          var historyPromotions = orderItem.promotions.map((e) => OrderItemPromotion.clone(e)).toList();
          ...
          for (var p in historyPromotions) {
            //删除数据库的行记录对应的赠送优惠记录
            queues.add("delete from pos_order_item_promotion where orderId='${orderItem.orderId}' and itemId='${orderItem.id}';");
          }

          for (var promotion in orderItem.promotions) {
            //添加行记录的赠送优惠pos_order_item_promotion表的SQL语句
            var orderItemPromotion = OrderUtils.instance.orderItemPromotionToSql(promotion, orderItem.finishDate);
  ...
  //退菜
  Future<Tuple2<bool, String>> changeRefundQuantity(OrderObject orderObject, OrderItem orderItem, List<OrderTable> tables, double refundQuantity, String refundReason) async {
    bool result = false;
    String message = "";
    try {
      var queues = new Queue<String>();
      ...
      for (var table in tables) {
        ...
        ///获取桌台的未下单商品的清单，桌台、规格、数量、批量添加标识、修改数量的商品ID
        var newOrderList = orderObject.items.where((x) => x.tableId == tableId && x.specId == specId && x.quantity == quantity && x.tableBatchTag == tableBatchTag && x.orderRowStatus == OrderRowStatus.Order).toList();
        ...
        for (var orderItem in newOrderList) {
          ...
          if (orderItem.quantity == orderItem.refundQuantity) {
            ...
            if ((orderItem.refundQuantity + orderItem.giftQuantity) > orderItem.quantity) {
              //删除数据库的行记录对应的赠送优惠记录
              queues.add("delete from pos_order_item_promotion where orderId='${orderItem.orderId}' and itemId='${orderItem.id}' and promotionType = '${PromotionType.Gift.value}';");
            }
          }
  ...
  //整单折扣
  Future<Tuple2<bool, String>> changeOrderDiscount(OrderObject orderObject, double discountRate, String discountReason, {bool restoreOriginalPrice = false}) async {
    bool result = false;
    String message = "";
    try {
      var queues = new Queue<String>();
      ...
      if (restoreOriginalPrice) {
        ...
        if (orderPromotion != null) {
          ...
          queues.add("delete from pos_order_promotion where id='$promotionId';");
          ...
          queues.add("delete from pos_order_item_promotion where relationId='$promotionId';");
        }
      } else {
        if (orderObject.promotions != null && orderObject.promotions.any((x) => x.promotionType == PromotionType.OrderDiscount)) {
          orderPromotion = orderObject.promotions.lastWhere((x) => x.promotionType == PromotionType.OrderDiscount);
        ...
        //先去除已经存在的整单折扣优惠
        queues.add("delete from pos_order_promotion where id='${orderPromotion.id}';");
        queues.add("delete from pos_order_item_promotion where relationId='${orderPromotion.id}';");

        //将重算后的整单折扣入库,pos_order_promotion表的SQL语句
        var orderPromotionSql = OrderUtils.instance.orderPromotionToSql(orderPromotion, orderObject.finishDate);
      ...
      for (var table in tables) {
        ...
        for (var orderItem in tableOrderItemList) {
          if (orderItem.promotions.any((x) => x.promotionType == PromotionType.OrderDiscount)) {
            var promotion = orderItem.promotions.lastWhere((x) => x.promotionType == PromotionType.OrderDiscount);

            var orderItemPromotionSql = OrderUtils.instance.orderItemPromotionToSql(promotion, orderObject.finishDate);
        ...
        //已经下单的商品数量
        table.placeOrders = orderObject.items.where((x) => x.tableId == tableId && x.orderRowStatus == OrderRowStatus.Order).map((e) => e.quantity).fold(0, (prev, quantity) => prev + quantity);
  ...
  //整单议价
  Future<Tuple2<bool, String>> changeOrderBargain(OrderObject orderObject, double inputAmount, String bargainReason, {bool restoreOriginalPrice = false}) async {
    bool result = false;
    String message = "";
    try {
      var queues = new Queue<String>();
      ...
      for (var table in tables) {
        ...
        //桌台的单品中包含整单折扣的优惠计入数据库
        for (var orderItem in tableOrderItemList) {
          if (orderItem.promotions.any((x) => x.promotionType == PromotionType.OrderBargain)) {
            var promotion = orderItem.promotions.lastWhere((x) => x.promotionType == PromotionType.OrderBargain);
