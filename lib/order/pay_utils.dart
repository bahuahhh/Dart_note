...
class PayUtils {
  ...
  ///发起扫码支付
  Future<Tuple3<bool, String, ScanPayResult>> scanPayResult(String payCode, OrderObject orderObject, {OnLinePayBusTypeEnum busType = OnLinePayBusTypeEnum.Sale}) async {
    //执行结果
    bool result = false;
    //提醒消息
    String message = "发起查询";
    ...
    try {
      ...
      //支付参数
      var payParameterResult = await OrderUtils.instance.getPayParameterByPayMode(currentPayMode, busType);
      //获取支付参数失败
      if (!payParameterResult.item1) {
        ...
        switch (paySign) {
              ...
              if (!payResult.item1) {
                ...
                if (payResult.item3) {
                  ...
                  if (!queryResult.item1) {
                    ...
                    for (int i = 0; i < 60; i++) {
                      queryResult = await LeshuaPayUtils.queryPayment(currentPayMode, payParameter, orderObject.tradeNo, payNo);
                      FLogger.info("第${i + 1}次查询支付状态:${queryResult.item1},${queryResult.item2}");
                      //扣款成功
                      if (queryResult.item1) {
                        //查询扣款结果成功
                        result = true;
              ...
              if (!payResult.item1) {
                ...
                if (payResult.item3) {
                  ...
                  if (!queryResult.item1) {
                    //第一次查询支付结果失败,每500毫秒执行一次，重试60次，大约30秒
                    for (int i = 0; i < 60; i++) {
                      queryResult = await SaobeiPayUtils.queryPayment(currentPayMode, payParameter, orderObject.tradeNo, payNo, voucherNo, payTime);
                      FLogger.info("第${i + 1}次查询支付状态:${queryResult.item1},${queryResult.item2}");
                      //扣款成功
                      if (queryResult.item1) {
                        //查询扣款结果成功
          ...
          case PayParameterSignEnum.XiaobeiPay:
            {
              FLogger.info("适配到小呗支付通道,支付码:$payCode,订单号:${orderObject.tradeNo},待支付金额:${orderObject.unreceivableAmount}");
              var payResult = await XiaobeiPayUtils.paymentResult(currentPayMode, payParameter, payCode, orderObject.tradeNo, orderObject.unreceivableAmount);
              //支付不成功
              if (!payResult.item1) {
                result = false;
                message = payResult.item2;
                ...
                if (payResult.item3) {
                  ...
                  var queryResult = await XiaobeiPayUtils.queryPayment(currentPayMode, payParameter, orderObject.tradeNo, payNo, voucherNo, payTime);

                  if (!queryResult.item1) {
                    //第一次查询支付结果失败,每500毫秒执行一次，重试60次，大约30秒
                    for (int i = 0; i < 60; i++) {
                      queryResult = await XiaobeiPayUtils.queryPayment(currentPayMode, payParameter, orderObject.tradeNo, payNo, voucherNo, payTime);
                      FLogger.info("第${i + 1}次查询支付状态:${queryResult.item1},${queryResult.item2}");
                      //扣款成功
                      if (queryResult.item1) {
                        //查询扣款结果成功
