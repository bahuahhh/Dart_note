...
class ShiftUtils {
  ...
  Future<Tuple3<bool, String, ShiftoverTicket>> saveShiftLog(ShiftLog shiftLog, List<ShiftoverTicketPay> orderPayList, ShiftoverTicketCash cashPayDetail, double shiftAmount, String memo) async {
    bool result = false;
    String message = "";
    ShiftoverTicket entity;
    try {
      var shiftTime = DateTimeUtils.formatDate(DateTime.now(), format: "yyyy-MM-dd HH:mm:ss");
      ...
      if (orderPayList != null && orderPayList.length > 0) {
        for (var x in orderPayList) {
          x.businessType = "0"; //ShiftBusinessType.POS销售;// 0-pos销售 1-微店销售 2-会员退卡 10-充值
