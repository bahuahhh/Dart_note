...
class PromotionUtils {
  ...
  /// 创建新的订单项促销
  OrderItemPromotion newOrderItemPromotion(OrderItem orderItem, PromotionType promotionType) {
  ...
  /// 创建新的订单促销
  OrderPromotion newOrderPromotion(OrderObject orderObject, {PromotionType promotionType = PromotionType.None}) {
  ...
  void calculate(OrderObject orderObject, OrderPromotion orderPromotion, {List<OrderItem> itemList}) {
    try {
      switch (orderPromotion.promotionType) {
        case PromotionType.Coupon:
          {
            List<OrderItem> fineDiscountItem = orderObject.items.where((x) => x.rowType != OrderItemRowType.Detail && x.rowType != OrderItemRowType.SuitDetail && x.foreDiscount == 1).toList();

            var discount = orderPromotion.discountRate;
            double discountAmountSum = 0;

            for (var item in fineDiscountItem) {
              OrderItemPromotion promotion = newOrderItemPromotion(item, orderPromotion.promotionType);
              ...
              promotion.onlineFlag = 0;
              promotion.bargainPrice = OrderUtils.instance.toRound(item.price * discount);
              ...
              discountAmountSum += OrderUtils.instance.toRound((item.price - promotion.bargainPrice) * _effectiveQuantity);
            }
        ...
        case PromotionType.OrderDiscount:
          {
            ...
            fineDiscountItem = orderObject.items.where((x) => !x.promotions.any((y) => y.promotionType == PromotionType.Gift) && x.rowType != OrderItemRowType.Detail && x.rowType != OrderItemRowType.SuitDetail).toList();

            //禁止前台打折的商品允许做整单折扣
            var noBargainElseSwitch = "0"; //DataCacheManager.GetLineSalesSetting("allow_order_bargin_for_bandiscount_product");
            if (noBargainElseSwitch == "0") {
              //不允许，需要排除不允许前台议价的商品金额
              fineDiscountItem = fineDiscountItem.where((x) => x.foreDiscount == 1).toList();
            }

            //参与促销优惠后的商品允许再做整单打折或议价是否启用 0-否，1-是
            var allowOrderDiscountForPromotion = "0"; //DataCacheManager.GetLineSalesSetting("allow_order_discount_for_promotioned_product");
            if (allowOrderDiscountForPromotion == "0") {
              fineDiscountItem = fineDiscountItem.where((x) => !x.promotions.any((y) => y.promotionType == PromotionType.OnlinePromotion)).toList();
            }

            FLogger.info("整单商品数量:${orderObject.items.length},参与整单折扣的商品清单:${fineDiscountItem.length}");
            ...
            if (fineDiscountItem != null && fineDiscountItem.length > 0) {
              for (var item in fineDiscountItem) {
                ...
                if (item.promotions != null && item.promotions.any((x) => x.promotionType == orderPromotion.promotionType)) {
                  //订单中包含整单折扣优惠时，先去掉整单折扣，再计算商品价格
                  item.promotions.removeWhere((x) => x.promotionType == orderPromotion.promotionType);
                  OrderUtils.instance.calculateOrderItem(item);
                }

                promotion = newOrderItemPromotion(item, orderPromotion.promotionType);
                item.promotions.add(promotion);

                promotion.onlineFlag = 0;
                promotion.reason = orderPromotion.reason;
                promotion.bargainPrice = OrderUtils.instance.toRound(item.price * discountRate);
              ...
              //没有能够适用的商品，删除该优惠
              orderObject.promotions.removeWhere((x) => x.id == orderPromotion.id);
            }
          }
          break;
        ...
        case PromotionType.OrderBargain:
          {
            ...
            //优惠金额减去赠品金额
            var giftOrderItem = orderObject.items.where((x) => x.promotions.any((y) => y.promotionType == PromotionType.Gift)).toList();
            double giftAmount = 0.0;
            if (giftOrderItem != null && giftOrderItem.length > 0) {
              giftAmount = giftOrderItem.map((e) => e.discountAmount).fold(0, (prev, amount) => prev + amount);
            }

            orderPromotion.discountAmount -= giftAmount;

            //如果订单包含赠品，分摊议价金额的时候需要去除赠品
            fineBargainItem = orderObject.items.where((x) => !x.promotions.any((y) => y.promotionType == PromotionType.Gift) && x.rowType != OrderItemRowType.Detail && x.rowType != OrderItemRowType.SuitDetail).toList();
            //禁止前台议价的商品允许做整单议价
            var noBargainElseSwitch = "0"; //DataCacheManager.GetLineSalesSetting("allow_order_bargin_for_bandiscount_product");
            if (noBargainElseSwitch == "0") {
              //不允许，需要排除不允许前台议价的商品金额
              fineBargainItem = fineBargainItem.where((x) => x.foreBargain == 1).toList();
            }

            //参与促销优惠后的商品允许再做整单议价是否启用 0-否，1-是
            var allowOrderDiscountForPromotion = "0"; //DataCacheManager.GetLineSalesSetting("allow_order_discount_for_promotioned_product");
            if (allowOrderDiscountForPromotion == "0") {
              fineBargainItem = fineBargainItem.where((x) => !x.promotions.any((y) => y.promotionType == PromotionType.OnlinePromotion)).toList();
            }
            ...
            FLogger.info("整单商品数量:${orderObject.items.length},参与整单议价的商品清单:${fineBargainItem.length}");
            ...
            for (var item in fineBargainItem) {
              ...
              if (i >= fineBargainItem.length) {
                //最后一个，做减法
                share = OrderUtils.instance.toRound(discountAmountSum - sharedAmountSum);
              } else {
              ...
              var discountPrice = OrderUtils.instance.toRound(share / effectiveQuantity);

              if (OrderUtils.instance.toRound(discountPrice * effectiveQuantity) > share) {
              ...
              var currentShare = OrderUtils.instance.toRound((item.salePrice - promotion.bargainPrice) * effectiveQuantity);
