...
import 'package:estore_app/enums/maling_enum.dart';
...
import 'package:estore_app/number/decimal.dart';
import 'package:estore_app/number/number.dart';
...
import 'package:estore_app/utils/cache_manager.dart';
...
class OrderUtils {
  ...
  void calculateOrderItem(OrderItem master, {bool allOrder = false}) {
    ...
    //修改日期
    master.modifyDate = DateTimeUtils.formatDate(DateTime.now(), format: "yyyy-MM-dd HH:mm:ss");
  }
      ...
      case PromotionType.ProductDiscount: //单品折扣
        {
          ...
          //操作日期
          promotion.modifyDate = DateTimeUtils.formatDate(DateTime.now(), format: "yyyy-MM-dd HH:mm:ss");
        }
        break;
  ...
  void calculateTable(OrderObject orderObject, OrderTable table) {
    ...
    double unWeightCount =
        orderObject.items.where((item) => item.tableId == table.tableId && item.rowType != OrderItemRowType.Detail && item.rowType != OrderItemRowType.SuitDetail && item.weightFlag == 0).map((e) => e.quantity).fold(0, (prev, quantity) => prev + quantity);
    double weightCount =
        orderObject.items.where((item) => item.tableId == table.tableId && item.rowType != OrderItemRowType.Detail && item.rowType != OrderItemRowType.SuitDetail && item.weightFlag == 1).map((e) => e.quantity).fold(0, (prev, quantity) => prev + quantity);
  ...
  Future<Tuple3<bool, String, String>> generateMergeNo({int length = 2}) async {
    ...
    try {
      ...
      String sql = sprintf(
          "select `id`, `tenantId`, `group`, `keys`, `initValue`, `values`, `createUser`, `createDate`, `modifyUser`, `modifyDate` from pos_config where `group` = '%s' and `keys` = '%s';", [ConfigConstant.GROUP_BUSINESS, ConfigConstant.TABLE_MERGE_NO]);
      Config config;
      var database = await SqlUtils.instance.open();
      List<Map<String, dynamic>> lists = await database.rawQuery(sql);
      if (lists != null && lists.length > 0) {
      ...
      if (config != null) {
        ...
        config.createDate = DateTimeUtils.formatDate(DateTime.now(), format: "yyyy-MM-dd HH:mm:ss");
      ...
      config.modifyDate = DateTimeUtils.formatDate(DateTime.now(), format: "yyyy-MM-dd HH:mm:ss");
  ...
  Future<Tuple3<bool, String, String>> generateTicketNo({int length = 4}) async {
    ...
    try {
      ...
      if (serialNumber.isNotEmpty) {
        //日期
        String yyMMddHHmm = DateTimeUtils.formatDate(DateTime.now(), format: "yyMMddHHmm");
  ...
  Future<Tuple3<bool, String, String>> generateShiftNo({int length = 4}) async {
    ...
    try {
      ...
      data = "3${DateTimeUtils.formatDate(DateTime.now(), format: "yyMMddHHmm")}${Global.instance.authc.posNo}$shiftNo";
  ...
  Future<OrderObject> addOrderPayByScanPayResult(OrderObject orderObject, ScanPayResult payResult) async {
    ...
    if (payResult != null && payResult.success) {
      ...
      if (payMode != null) {
        ...
        pay.createDate = DateTimeUtils.formatDate(DateTime.now(), format: "yyyy-MM-dd HH:mm:ss");
  ...
  Future<Tuple3<bool, String, String>> buildOrderObjectSql(OrderObject orderObject, String finishDate) async {
    ...
    try {
      ...
      data = sprintf(template, [
        ...
        orderObject.createDate ?? DateTimeUtils.formatDate(DateTime.now(), format: "yyyy-MM-dd HH:mm:ss"),
        orderObject.refundStatus.value,
