...
// 工厂模式
  ...
  Tuple2<bool, String> beforeMenuActionValidate(OrderObject orderObject, OrderItem orderItem, ModuleKeyCode moduleKeyCode) {
    // 第一步，菜单是否为操作当前订单，不是则放行，是则校验当前订单是否还允许操作
    var result = beforeOrderObjectValidateStep1(orderObject, orderItem, moduleKeyCode);
    if (result.item1) {
      result = beforeOrderObjectValidateStep2(orderObject, orderItem, moduleKeyCode);
    }
  ...
  //第一步，菜单是否为操作当前订单，不是则放行，是则校验当前订单是否还允许操作
  Tuple2<bool, String> beforeOrderObjectValidateStep1(OrderObject orderObject, OrderItem orderItem, ModuleKeyCode moduleKeyCode) {
    bool result = true;
    String message = "校验通过";
  ...
  //第二步，菜单操作是否违反具体业务规则
  Tuple2<bool, String> beforeOrderObjectValidateStep2(OrderObject orderObject, OrderItem orderItem, ModuleKeyCode moduleKeyCode) {
    bool result = true;
    String message = "校验通过";
  ...
  ///桌台模式的菜单操作前的校验
  Tuple2<bool, String> beforeTableMenuActionValidate(OrderObject orderObject, OrderItem orderItem, ModuleKeyCode moduleKeyCode) {
    // 第一步，菜单是否为操作当前订单，不是则放行，是则校验当前订单是否还允许操作
    var result = beforeTableOrderObjectValidateStep1(orderObject, orderItem, moduleKeyCode);
    if (result.item1) {
      result = beforeTableOrderObjectValidateStep2(orderObject, orderItem, moduleKeyCode);
    }
  ...
  //第一步，菜单是否为操作当前订单，不是则放行，是则校验当前订单是否还允许操作
  Tuple2<bool, String> beforeTableOrderObjectValidateStep1(OrderObject orderObject, OrderItem orderItem, ModuleKeyCode moduleKeyCode) {
    bool result = true;
    String message = "校验通过";
  ...
  //第二步，菜单操作是否违反具体业务规则
  Tuple2<bool, String> beforeTableOrderObjectValidateStep2(OrderObject orderObject, OrderItem orderItem, ModuleKeyCode moduleKeyCode) {
    bool result = true;
    String message = "校验通过";
    ...
    switch (moduleKeyCode) {
      ...
      case ModuleKeyCode.$_122: //退货
        {
          ...
          if (result && (orderItem.orderRowStatus == OrderRowStatus.New || orderItem.orderRowStatus == OrderRowStatus.Save)) {
            message = "商品未下单,请直接删除";
            result = false;
          }
        }
      ...
      case ModuleKeyCode.$_111: //做法
        {
          //单据状态判断
          if (result && orderObject.orderStatus == OrderStatus.Completed || orderObject.orderStatus == OrderStatus.ChargeBack) {
            result = false;
            message = "订单已结账";
          }
