...
class OrderItem extends Equatable {
  ...
  /// 行记录的创建时间
  String saleDate = DateTimeUtils.formatDate(DateTime.now(), format: "yyyy-MM-dd HH:mm:ss");
  ...
  factory OrderItem.newOrderItem(OrderObject orderObject, ProductExt product, OrderItemJoinType joinType, {OrderRowStatus orderRowStatus = OrderRowStatus.None}) {
    return OrderItem()
      //ID
  ...
  factory OrderItem.fromJson(Map<String, dynamic> map) {
    OrderItem result = OrderItem()
      ...
      //去券后总应收
      ..totalReceivableRemoveCouponAmount = Convert.toDouble(map["totalReceivableRemoveCouponAmount"])
      //去券占用金额后总应收
      ..totalReceivableRemoveCouponLeastCost = Convert.toDouble(map["totalReceivableRemoveCouponLeastCost"])
      //支付实收，支付方式中是否实收为是的支付合计
      ...
      //行状态
      ..orderRowStatus = OrderRowStatus.fromValue(Convert.toStr(map["orderRowStatus"]))
      ..promotions =
          map["promotions"] != null ? List<OrderItemPromotion>.from(List<Map<String, dynamic>>.from(map["promotions"] is String ? json.decode(map["promotions"]) : map["promotions"]).map((x) => OrderItemPromotion.fromJson(x))) : <OrderItemPromotion>[];

    var product = ProductExt.clone(CacheManager.productExtList.lastWhere((x) => x.id == result.productId));
  ...
  factory OrderItem.clone(OrderItem obj) {
    return OrderItem()
      ...
      //去券后总应收
      ..totalReceivableRemoveCouponAmount = obj.totalReceivableRemoveCouponAmount
      //去券占用金额后总应收
      ..totalReceivableRemoveCouponLeastCost = obj.totalReceivableRemoveCouponLeastCost
      //支付实收，支付方式中是否实收为是的支付合计
      ...
      //单品享受的优惠列表
      ..promotions = obj.promotions.map((item) => OrderItemPromotion.clone(item)).toList()
      //做法/要求明细
  ...
  Map<String, dynamic> toJson() {
    Map<String, dynamic> map = {
      ...
      //去券后总应收
      "totalReceivableRemoveCouponAmount": this.totalReceivableRemoveCouponAmount,
      //去券占用金额后总应收
      "totalReceivableRemoveCouponLeastCost": this.totalReceivableRemoveCouponLeastCost,
      //支付实收，支付方式中是否实收为是的支付合计
