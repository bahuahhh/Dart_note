...
import 'package:estore_app/payment/scan_pay_result.dart';
import 'package:estore_app/payment/xiaobei/xiaobei_constant.dart';
import 'package:estore_app/payment/xiaobei/xiaobei_payment_result.dart';
import 'package:estore_app/payment/xiaobei/xiaobei_query_result.dart';
import 'package:estore_app/payment/xiaobei/xiaobei_refund_result.dart';
import 'package:estore_app/utils/converts.dart';
import 'package:estore_app/utils/crypto_utils.dart';
import 'package:estore_app/utils/hex_codec.dart';
...
class XiaobeiPayUtils {
  ...
  static Future<Tuple4<bool, String, bool, ScanPayResult>> paymentResult(PayMode payMode, PaymentParameter payParameter, String payCode, String tradeNo, double amount) async {
    //执行结果
    bool result = false;
    //提醒消息
    String message = "发起支付";
    ...
    try {
      ...
      String open_id = parameter.containsKey("open_id") ? Convert.toStr(parameter["open_id"]) : "";
      String open_key = parameter.containsKey("open_key") ? Convert.toStr(parameter["open_key"]) : "";
      String url = parameter.containsKey("url") ? Convert.toStr(parameter["url"]) : "";
      String shop_pass = parameter.containsKey("shop_pass") ? Convert.toStr(parameter["shop_pass"]) : "";
      ...
      keyValues["out_no"] = OrderUtils.instance.generatePayNo(tradeNo, separator: "");
      //付款方式编号
      ...
      FLogger.info("订单[${keyValues["out_no"]}]发起江西农商银行支付（$payWay）支付:<${keyValues["trade_amount"]}>分");
      ...
      FLogger.info("订单[${keyValues["out_no"]}]收到江西农商银行支付$payWay结果:$responseBody");
      ...
      if (responseBody.containsKey("errcode")) {
        ...
        if (errcode == "0") {
          ...
          if (responseBody.containsKey("data")) {
            ...
            if (XiaobeiPayUtils.verifySign(SplayTreeMap.from(responseBody), open_key)) {
              var encrptData = responseBody["data"];
              var decodeData = XiaobeiPayUtils.dataDecode(encrptData, open_key);
              FLogger.info("订单[${keyValues["out_no"]}]收到江西农商银行支付$payWay结果:$decodeData");
              var data = json.decode(decodeData);
              if (data != null) {
                //处理应答结果
                xiaobeiPaymentResult = XiaobeiPaymentResult.fromPaymentJson(errcode, errormsg, data);
                ...
                FLogger.info("订单[${keyValues["out_no"]}]收到江西农商银行支付结果未能转化为有效的json对象");
                result = false;
                isQuery = false;
                message = "应答数据非法";
              }
              ...
              isQuery = false;
              //验签失败
              FLogger.info("订单[${keyValues["out_no"]}]收到江西农商银行支付$payWay支付结果:验签失败");
            }
          } else {
            result = false;
            message = "缺失data标识";
            isQuery = false;
            //验签失败
            FLogger.info("订单[${keyValues["out_no"]}]收到江西农商银行支付$payWay支付结果:$message");
          }
        } else {
          result = false;
        ...
        isQuery = false;
        //验签失败
        FLogger.info("订单[${keyValues["out_no"]}]收到$payWay支付结果:请求应答报文非法,缺失errcode标识");
      }
    } catch (e, stack) {
      result = false;
      message = "支付发生错误";
    ...
    //支付成功，并且也返回了成功的状态
    if (result) {
      payResult = ScanPayResult.fromXiaobeiPaymentResult(xiaobeiPaymentResult, tradeNo, payMode);
    }

    return Tuple4(result, message, isQuery, payResult);
  }

  static Future<Tuple3<bool, String, ScanPayResult>> queryPayment(PayMode payMode, PaymentParameter payParameter, String tradeNo, String payNo, String voucherNo, String payTime) async {
    //执行结果
    bool result = false;
    //提醒消息
    String message = "发起查询";
        ...
        if (errcode == "0") {
          ...
          if (responseBody.containsKey("data")) {
            if (XiaobeiPayUtils.verifySign(SplayTreeMap.from(responseBody), open_key)) {
              var encrptData = responseBody["data"];
              var decodeData = XiaobeiPayUtils.dataDecode(encrptData, open_key);
              FLogger.info("订单[${keyValues["out_no"]}]收到江西农商银行支付查询结果:$decodeData");
              var data = json.decode(decodeData);
              if (data != null) {
                //处理应答结果
                xiaobeiQueryResult = XiaobeiQueryResult.fromQueryJson(errcode, errormsg, data);
                ...
                FLogger.info("订单[${keyValues["out_no"]}]收到江西农商银行支付查询结果未能转化为有效的对象");
                result = false;
                message = "应答数据非法";
              }
            } else {
    ...
    //支付成功，并且也返回了成功的状态
    if (result) {
      payResult = ScanPayResult.fromXiaoaobeiQueryResult(xiaobeiQueryResult, tradeNo, payMode);
    }

    return Tuple3(result, message, payResult);
  }

  static Future<Tuple4<bool, String, bool, ScanPayResult>> refundResult(PayMode payMode, PaymentParameter payParameter, String payNo, String tradeNo, double refundAmound) async {
    //执行结果
    bool result = false;
    //提醒消息
    String message = "发起退单";
    ...
    try {
      ...
      String open_key = parameter.containsKey("open_key") ? Convert.toStr(parameter["open_key"]) : "";
      String url = parameter.containsKey("url") ? Convert.toStr(parameter["url"]) : "";
      String shop_pass = parameter.containsKey("shop_pass") ? Convert.toStr(parameter["shop_pass"]) : "123456";
      ...
      param["data"] = XiaobeiPayUtils.dataEncrypt(jsonString, open_key);
      param["sign"] = XiaobeiPayUtils.sign(param, open_key);
      FLogger.info("订单[${keyValues["out_no"]}]发起退款:<${keyValues["refund_amount"]}>分");
        ...
        if (errcode == "0") {
          ...
          if (responseBody.containsKey("data")) {
            if (XiaobeiPayUtils.verifySign(SplayTreeMap.from(responseBody), open_key)) {
              ...
              if (data != null) {
                //处理应答结果
                xiaobeiRefundResult = XiaobeiRefundResult.fromRefundJson(errcode, errormsg, data);
    ...
    //支付成功，并且也返回了成功的状态
    if (result) {
      payResult = ScanPayResult.fromXiaobeiRefundResult(xiaobeiRefundResult, tradeNo, payMode);
    }
  ...
  static SplayTreeMap<String, dynamic> initParameters() {
    SplayTreeMap<String, dynamic> parameters = new SplayTreeMap<String, dynamic>();
  ...
  static bool verifySign(SplayTreeMap<String, dynamic> param, String openKey) {
    ...
    //去除sign 排序
    SplayTreeMap<String, dynamic> sortList = new SplayTreeMap<String, dynamic>();
  ...
  static String sign(SplayTreeMap<String, dynamic> parameters, String openKey) {
    ...
    String parameterStr = str.toString().substring(0, str.toString().length - 1);
