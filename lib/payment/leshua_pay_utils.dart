...
class LeshuaPayUtils {
  ...
  static Future<Tuple4<bool, String, bool, ScanPayResult>> paymentResult(PayMode payMode, PaymentParameter payParameter, String payCode, String tradeNo, double amount) async {
    ...
    try {
      ...
      //支付参数
      String channel = parameter.containsKey("channel") ? Convert.toStr(parameter["channel"]) : "";
      String merchantId = parameter.containsKey("merchant_id") ? Convert.toStr(parameter["merchant_id"]) : "";
      String payKey = parameter.containsKey("payKey") ? Convert.toStr(parameter["payKey"]) : "";
      String notifyKey = parameter.containsKey("notifyKey") ? Convert.toStr(parameter["notifyKey"]) : "";
      ...
      //商户订单号
      keyValues["third_order_id"] = OrderUtils.instance.generatePayNo(tradeNo, separator: "");
      //订单金额
      ...
      //支付成功
      if (leshuaPaymentResult != null && leshuaPaymentResult.resp_code == "0" && leshuaPaymentResult.result_code == "0") {
        result = true;
        ...
        result = false;
        message = "${leshuaPaymentResult.resp_msg}<${leshuaPaymentResult.resp_code}>";
      }
    } catch (e, stack) {
      result = false;
      message = "支付发生错误";
    ...
    //支付成功，并且也返回了成功的状态
    if (result) {
      payResult = ScanPayResult.fromLeShuaPaymentResult(leshuaPaymentResult, tradeNo, payMode);
    }
  ...
  static Future<Tuple3<bool, String, ScanPayResult>> queryPayment(PayMode payMode, PaymentParameter payParameter, String tradeNo, String payNo) async {
    //执行结果
    bool result = false;
    //提醒消息
    String message = "发起查询";
    ...
    try {
      ...
      //支付成功
      if (leshuaQueryResult != null && leshuaQueryResult.resp_code == "0" && leshuaQueryResult.result_code == "0") {
        ...
        result = false;
        message = "${leshuaQueryResult.resp_msg}<${leshuaQueryResult.resp_code}>";
      }
    } catch (e, stack) {
      result = false;
      message = "支付发生错误";
    ...
    //支付成功，并且也返回了成功的状态
    if (result) {
      payResult = ScanPayResult.fromLeShuaQueryResult(leshuaQueryResult, tradeNo, payMode);
    }
  ...
  static Future<Tuple4<bool, String, bool, ScanPayResult>> refundResult(PayMode payMode, PaymentParameter payParameter, String tradeNo, String payNo, double amount) async {
    //执行结果
    bool result = false;
    //提醒消息
    String message = "发起退单";
    ...
    try {
      ...
      if (leshuaPaymentResult != null && leshuaPaymentResult.resp_code == "0" && leshuaPaymentResult.result_code == "0") {
        ...
        result = false;
        message = "${leshuaPaymentResult.error_msg}<${leshuaPaymentResult.error_code}>";
      }
    } catch (e, stack) {
      result = false;
      message = "退款发生错误";
  ...
  static String sign(SplayTreeMap<String, String> paramsMap, String key) {
    ...
    for (var key in paramsMap.keys) {
      ...
      if (StringUtils.isBlank(pValue) || pKey == "leshua" || pKey == "resp_code" || pKey == "sign") {
        continue;
      }
      str.write(pKey + "=" + pValue + "&");
    }
