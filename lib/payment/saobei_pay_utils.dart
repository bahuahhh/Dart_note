...
import 'package:estore_app/payment/saobei/saobei_payment_result.dart';
import 'package:estore_app/payment/saobei/saobei_query_result.dart';
...
class SaobeiPayUtils {
  ...
  static Future<Tuple4<bool, String, bool, ScanPayResult>> paymentResult(PayMode payMode, PaymentParameter payParameter, String payCode, String tradeNo, double amount) async {
    ...
    try {
      ...
      String merchant_no = parameter.containsKey("merchant_no") ? Convert.toStr(parameter["merchant_no"]) : "";
      String signKey = parameter.containsKey("signKey") ? Convert.toStr(parameter["signKey"]) : "";
      String terminal_id = parameter.containsKey("terminal_id") ? Convert.toStr(parameter["terminal_id"]) : "";
      String gatewayUrl = parameter.containsKey("gatewayUrl") ? Convert.toStr(parameter["gatewayUrl"]) : "";
      ...
      keyValues["terminal_trace"] = OrderUtils.instance.generatePayNo(tradeNo, separator: "");
      keyValues["terminal_time"] = DateTimeUtils.formatDate(DateTime.now(), format: "yyyyMMddHHmmss");
      ...
      var response = await http.post(requestUrl, headers: {"content-type": "application/json"}, body: jsonRequest, encoding: Encoding.getByName("utf-8"));
      ...
      //支付成功
      if (saobeiPaymentResult != null && saobeiPaymentResult.return_code == "01") {
        // 01成功 02失败 ，03支付中，99该条码暂不支持支付类型自动匹配 01的成功就是支付成功
        var reusltCode = saobeiPaymentResult.result_code;
        if ("01" == reusltCode) {
          result = true;
          isQuery = true;
          message = "${saobeiPaymentResult.return_msg}<${saobeiPaymentResult.return_code}>";
    ...
    //支付成功，并且也返回了成功的状态
    if (result) {
      payResult = ScanPayResult.fromSaobeiPaymentResult(saobeiPaymentResult, tradeNo, payMode);
    }
  ...
  static Future<Tuple3<bool, String, ScanPayResult>> queryPayment(PayMode payMode, PaymentParameter payParameter, String tradeNo, String payNo, String voucherNo, String payTime) async {
    ...
    try {
      ...
      if (saobeiQueryResult != null && saobeiQueryResult.return_code == "01") {
        ...
        if ("01" == reusltCode) {
          result = true;
          message = "${saobeiQueryResult.return_msg}<${saobeiQueryResult.return_code}>";
        } else if ("02" == reusltCode) {
          result = false;
          message = "${saobeiQueryResult.return_msg}<${saobeiQueryResult.return_code}>";
        } else if ("03" == reusltCode) {
    ...
    //支付成功，并且也返回了成功的状态
    if (result) {
      payResult = ScanPayResult.fromSaobeiQueryResult(saobeiQueryResult, tradeNo, payMode);
    }
  ...
  static Future<Tuple4<bool, String, bool, ScanPayResult>> refundResult(PayMode payMode, PaymentParameter payParameter, String payNo, String tradeNo, double refundAmound) async {
    ...
    //支付成功，并且也返回了成功的状态
    if (result) {
      payResult = ScanPayResult.fromSaobeiRefundResult(saobeiPaymentResult, tradeNo, payMode);
    }

    return Tuple4(result, message, isQuery, payResult);
  }

  static String sign(String access_token, Map<String, String> parameters, List<String> ignoreParameters) {
