...
class AssistantGiftDialog extends StatefulWidget {
  // 订单对象
  ...
  AssistantGiftDialog(this.orderObject, this.orderItem, this.permissionCode, {this.onAccept, this.onClose});

  @override
  _AssistantGiftDialogState createState() => _AssistantGiftDialogState();
}

class _AssistantGiftDialogState extends State<AssistantGiftDialog> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((callback) {
      ///可赠送数量
      double giftQuantity = widget.orderItem.quantity - widget.orderItem.refundQuantity;
      final text = giftQuantity.toString();
  ...
  Widget _buildContent(AssistantState state) {
    return Container(
      ...
      height: Constants.getAdapterHeight(850),
        ...
        children: <Widget>[
          ...
          Container(
            ...
            child: Column(
              ...
              children: <Widget>[
                Text("赠送原因", style: TextStyles.getTextStyle(fontSize: 32)),
                Space(height: Constants.getAdapterHeight(20)),
                Expanded(
          ...
          FlatButton(
            ...
            onPressed: () {
              ...
              var args = new TableGiftArgs(orderItem, 0, "", cancelGift: true);
    ...
    return ConstrainedBox(
      ...
      child: TextFormField(
        ...
        onFieldSubmitted: (inputValue) async {
          ...
          if (orderItem == null) {
            ToastUtils.show("赠送当前不可用");
            FocusScope.of(context).requestFocus(_focus);
            return;
          }

          if ((orderItem.quantity - orderItem.refundQuantity) == orderItem.giftQuantity) {
            ToastUtils.show("已经全部赠送了");
            FocusScope.of(context).requestFocus(_focus);
            return;
          }

          if (StringUtils.isBlank(inputValue)) {
            ToastUtils.show("请输入赠送数量");
            FocusScope.of(context).requestFocus(_focus);
            return;
          }

          double giftQuantity = Convert.toDouble(inputValue);
          String giftReason = "${state.reasonSelected.name}";
          ...
          //可赠数量 = 数量 - 退量
          var _effectiveQuantity = orderItem.quantity - orderItem.refundQuantity;
          if (giftQuantity > _effectiveQuantity) {
            ToastUtils.show("输入无效,最大可赠数量为$_effectiveQuantity");
            FocusScope.of(context).requestFocus(_focus);
            return;
          }

          FocusScope.of(context).requestFocus(_focus);
          var args = new TableGiftArgs(orderItem, giftQuantity, giftReason);
          if (widget.onAccept != null) {
            widget.onAccept(args);
          }
        },
        ...
        children: <Widget>[
          Expanded(
            child: Container(
              ...
              child: Text("赠送", style: TextStyles.getTextStyle(color: Constants.hexStringToColor("#333333"), fontSize: 32, fontWeight: FontWeight.bold)),
            ),
          ),
          InkWell(
            onTap: () {
