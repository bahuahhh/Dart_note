...
///现金支付方式
class CashPayPage extends StatefulWidget {
  final String title;
  ...
  CashPayPage(this.title, this.orderObject, this.payMode, {this.onAccept, this.onClose});

  @override
  _CashPayPageState createState() => _CashPayPageState();
}

class _CashPayPageState extends State<CashPayPage> with SingleTickerProviderStateMixin {
  //输入框
  ...
  Widget _buildContent(CashierState cashierState) {
    ...
    cashierState.orderObject.pays.forEach((item) {
    ...
    return ConstrainedBox(
      ...
      child: TextFormField(
        ...
        onFieldSubmitted: (inputValue) async {
          ...
          orderPay.amount = changeAmount > 0 ? orderObject.receivableAmount : orderPay.inputAmount;
          orderPay.paidAmount = changeAmount > 0 ? orderObject.paidAmount : orderPay.inputAmount;
