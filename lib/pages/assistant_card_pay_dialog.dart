...
///会员支付方式
class AssistantCardPayDialog extends StatefulWidget {
  final String title;
  ...
  AssistantCardPayDialog(this.title, this.orderObject, this.payMode, {this.onAccept, this.onClose});

  @override
  _AssistantCardPayDialogState createState() => _AssistantCardPayDialogState();
}

class _AssistantCardPayDialogState extends State<AssistantCardPayDialog> with SingleTickerProviderStateMixin {
  //输入框
  ...
  Widget _buildContent(AssistantState state) {
    return Container(
      padding: Constants.paddingLTRB(20, 5, 20, 20),
          ...
          Container(
            ...
            child: Row(
              ...
              children: [
                ...
                Conditional.single(
                  context: context,
                  conditionBuilder: (BuildContext context) => StringUtils.isNotBlank(state.member.defaultCard.cardFaceNo),
                  widgetBuilder: (BuildContext context) => RichText(
                    text: TextSpan(text: "卡号:", style: TextStyles.getTextStyle(fontSize: 32, color: Constants.hexStringToColor("#333333")), children: <TextSpan>[
                      TextSpan(text: "${state.member.defaultCard.cardFaceNo}", style: TextStyles.getTextStyle(fontSize: 42, color: Constants.hexStringToColor("#333333"))),
                    ]),
                  ),
                  fallbackBuilder: (BuildContext context) => Conditional.single(
                    context: context,
                    conditionBuilder: (BuildContext context) => StringUtils.isNotBlank(state.member.defaultCard.mobile),
                    widgetBuilder: (BuildContext context) => RichText(
                      text: TextSpan(text: "卡号:", style: TextStyles.getTextStyle(fontSize: 32, color: Constants.hexStringToColor("#333333")), children: <TextSpan>[
                        TextSpan(text: "${state.member.defaultCard.mobile}", style: TextStyles.getTextStyle(fontSize: 42, color: Constants.hexStringToColor("#333333"))),
                      ]),
                    ),
                    fallbackBuilder: (BuildContext context) => RichText(
                      text: TextSpan(text: "卡号:", style: TextStyles.getTextStyle(fontSize: 32, color: Constants.hexStringToColor("#333333")), children: <TextSpan>[
                        TextSpan(text: "${state.member.defaultCard.cardNo}", style: TextStyles.getTextStyle(fontSize: 42, color: Constants.hexStringToColor("#333333"))),
                      ]),
                    ),
                  ),
                ),
                ...
                //     TextSpan(text: "${state.orderObject.member.defaultCard.totalAmount}", style: TextStyles.getTextStyle(fontSize: 32, color: Constants.hexStringToColor("#333333"))),
                //   ]),
                // ),
              ],
            ),
          ...
          Container(
            ...
            child: Row(
              ...
              children: [
                ...
                RichText(
                  text: TextSpan(text: "可用余额:¥", style: TextStyles.getTextStyle(fontSize: 32, color: Constants.hexStringToColor("#333333")), children: <TextSpan>[
                    TextSpan(text: "${state.member.defaultCard.totalAmount}", style: TextStyles.getTextStyle(fontSize: 46, color: Constants.hexStringToColor("#333333"))),
                  ]),
                ),
              ],
            ),
    ...
    return ConstrainedBox(
      ...
      child: TextFormField(
        ...
        onFieldSubmitted: (inputValue) async {
          ...
          //应付金额
          double unreceivableAmount = state.orderObject.unreceivableAmount;
