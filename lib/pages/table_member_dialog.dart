...
class TableMemberDialog extends StatefulWidget {
  ...
  TableMemberDialog(this.orderObject, {this.onAccept, this.onClose, this.onCallback});

  @override
  _TableMemberDialogState createState() => _TableMemberDialogState();
}

class _TableMemberDialogState extends State<TableMemberDialog> with SingleTickerProviderStateMixin {
  //会员识别输入
        ...
        children: <Widget>[
          Expanded(
            child: Container(
              ...
              child: Text("会员认证", style: TextStyles.getTextStyle(color: Constants.hexStringToColor("#333333"), fontSize: 32, fontWeight: FontWeight.bold)),
            ),
          ),
          InkWell(
            onTap: () {
  ...
  Future<void> _getMemberInfo(String code) async {
    //查询服务端会员信息
    ...
    if (queryResult.item1) {
      ...
      if (widget.onAccept != null) {
        var args = TableMemberArgs(member);
...
class TableMemberViewDialog extends StatefulWidget {
  final OrderObject orderObject;
  ...
  TableMemberViewDialog(this.orderObject, {this.onChanged, this.onCancel, this.onClose});

  @override
  _TableMemberViewDialogState createState() => _TableMemberViewDialogState();
}

class _TableMemberViewDialogState extends State<TableMemberViewDialog> with SingleTickerProviderStateMixin {
  TableCashierBloc _tableCashierBloc;
                    ...
                    child: Row(
                      ...
                      children: <Widget>[
                        Align(
                          ...
                          child: (state.member == null || StringUtils.isBlank(state.member.headImgUrl))
                              ? CircleAvatar(
                        ...
                        Expanded(
                          child: Column(
                            ...
                            crossAxisAlignment: CrossAxisAlignment.start,
                            children: <Widget>[
                              Text("${state.member == null ? "--" : state.member.name}", style: TextStyles.getTextStyle(color: Constants.hexStringToColor("#333333"), fontSize: 32)),
                              Space(height: Constants.getAdapterHeight(10)),
                              Text("电话：${state.member == null ? "--" : state.member.mobile}", style: TextStyles.getTextStyle(color: Constants.hexStringToColor("#333333"), fontSize: 28)),
                              Space(height: Constants.getAdapterHeight(10)),
                              Text("卡号：${state.member == null ? "--" : state.member.defaultCard.cardNo}", style: TextStyles.getTextStyle(color: Constants.hexStringToColor("#333333"), fontSize: 28)),
                              Space(height: Constants.getAdapterHeight(10)),
                              Text("生日：${state.member == null ? "--" : state.member.birthday ?? ''}", style: TextStyles.getTextStyle(color: Constants.hexStringToColor("#333333"), fontSize: 28)),
                            ],
                          ),
                        ),
                      ],
                        ...
                        child: Container(
                          child: Column(
                            children: <Widget>[
                              ...
                              Expanded(
                                child: Center(
                                  child: Text("${state.member == null ? "--" : state.member.totalAmount}", style: TextStyles.getTextStyle(color: Constants.hexStringToColor("#333333"), fontSize: 32)),
                                ),
                              ),
                            ],
                          ),
                        ...
                        child: Container(
                          child: Column(
                            children: <Widget>[
                              ...
                              Expanded(
                                child: Center(
                                  child: Text("${state.member == null ? "--" : state.member.totalPoint}", style: TextStyles.getTextStyle(color: Constants.hexStringToColor("#333333"), fontSize: 32)),
                                ),
                              ),
                            ],
                          ),
                        ...
                        child: Container(
                          child: Column(
                            children: <Widget>[
                              ...
                              Expanded(
                                child: Center(
                                  child: Text("${state.member == null ? "--" : state.member.memberLevelName}", style: TextStyles.getTextStyle(color: Constants.hexStringToColor("#333333"), fontSize: 32)),
                                ),
                              ),
                            ],
                          ),
        ...
        children: <Widget>[
          Expanded(
            child: Container(
              padding: Constants.paddingOnly(left: 15),
              alignment: Alignment.centerLeft,
              child: Text("会员信息", style: TextStyles.getTextStyle(color: Constants.hexStringToColor("#333333"), fontSize: 32, fontWeight: FontWeight.bold)),
            ),
          ),
          InkWell(
            onTap: () {
