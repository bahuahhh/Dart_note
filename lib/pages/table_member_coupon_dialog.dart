...
class TableMemberCouponDialog extends StatefulWidget {
  ...
  TableMemberCouponDialog(this.orderObject, this.couponList, this.couponSelected, {this.onAccept, this.onClose});

  @override
  _TableMemberCouponDialogState createState() => _TableMemberCouponDialogState();
}

class _TableMemberCouponDialogState extends State<TableMemberCouponDialog> with SingleTickerProviderStateMixin {
  //业务逻辑处理
  ...
  void initState() {
    ...
    //加载优惠券列表
    _tableCashierBloc.add(SelectCoupon(widget.orderObject, couponSelected: widget.couponSelected));
  ...
  Widget _buildContent(TableCashierState state) {
    return Container(
      padding: Constants.paddingLTRB(15, 5, 15, 5),
      height: Constants.getAdapterHeight(720),
        ...
        children: <Widget>[
          Expanded(
            child: GridView.builder(
              padding: Constants.paddingAll(0),
              itemCount: state?.member?.couponList?.length,
              itemBuilder: (BuildContext context, int index) {
                var coupon = state.member.couponList[index];
                var selected = state.couponSelected.any((x) => x.couponNo == coupon.couponNo);
                return Container(
                          ...
                          child: InkWell(
                            onTap: coupon.enable
                                ? () async {
                                    ...
                                    //会员全部优惠券
                                    List<MemberElecCoupon> couponList = state.member.couponList ?? <MemberElecCoupon>[];
                                    //会员已选优惠券清单
                                    List<MemberElecCoupon> couponSelected = (state.couponSelected ?? <MemberElecCoupon>[]).map((e) => MemberElecCoupon.clone(e)).toList();
                                    ...
                                    this._tableCashierBloc.add(SelectCoupon(
                                          orderObject,
        ...
        children: <Widget>[
          Expanded(
            child: Container(
              ...
              child: Text("选择优惠券", style: TextStyles.getTextStyle(color: Constants.hexStringToColor("#333333"), fontSize: 32, fontWeight: FontWeight.bold)),
            ),
          ),
          InkWell(
            onTap: () {
        ...
        children: <Widget>[
          FlatButton(
            child: Container(
              ...
              child: Text("取消", style: TextStyles.getTextStyle(fontSize: 32, color: Constants.hexStringToColor("#333333"))),
            ),
