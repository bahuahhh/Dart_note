...
//加载做法和规格页面
class SelectSpecAndMakePage extends StatefulWidget {
  ...
  SelectSpecAndMakePage(this.product, this.specList, this.makeList, {this.onAccept, this.onClose});

  @override
  _SelectSpecAndMakePageState createState() => _SelectSpecAndMakePageState();
}

class _SelectSpecAndMakePageState extends State<SelectSpecAndMakePage> with SingleTickerProviderStateMixin {
  //业务逻辑处理
  ...
  void initState() {
    ...
    //加载规格和做法原因
    _tableCashierBloc.add(SelectProductSpecAndMake(specList: widget.specList, makeList: widget.makeList));
  ...
  ///构建商品做法
  Widget _buildProductMake(TableCashierState state, MakeInfo make, bool selected) {
  ...
  ///构建商品规格
  Widget _buildProductSpec(TableCashierState state, ProductSpec item, bool selected) {
    var backgroundColor = selected ? Constants.hexStringToColor("#F8F7FF") : Constants.hexStringToColor("#FFFFFF");
    var borderColor = selected ? Constants.hexStringToColor("#7A73C7") : Constants.hexStringToColor("#D0D0D0");
    var titleColor = selected ? Constants.hexStringToColor("#7A73C7") : Constants.hexStringToColor("#333333");
    return InkWell(
  ...
  ///不同规格和做法金额试算
  Widget _buildCalculateAmount(TableCashierState state, bool showSpec) {
        ...
        children: <Widget>[
          ...
          Container(
            ...
            child: SpinnerInput(
              ...
              onChange: (newValue) {
                this._tableCashierBloc.add(SelectProductSpecAndMake(
                      specList: state.specList,
                      specSelected: state.specSelected,
                      makeList: state.makeList,
                      makeSelected: state.makeSelected,
  ...
  List<Widget> _buildOrderItemRow(TableCashierState state, bool showSpec) {
