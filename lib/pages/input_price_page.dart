...
class InputPricePage extends StatefulWidget {
  final ProductExt product;

  final OnAcceptCallback onAccept;
  final OnCloseCallback onClose;

  InputPricePage(this.product, {this.onAccept, this.onClose});

  @override
  _InputPricePageState createState() => _InputPricePageState();
}

class _InputPricePageState extends State<InputPricePage> with SingleTickerProviderStateMixin {
  //输入
      ...
      child: TextFormField(
        ...
        onFieldSubmitted: (inputValue) async {
          if (StringUtils.isBlank(inputValue)) {
            ToastUtils.show("请输入售价", position: ToastPosition.center);
            FocusScope.of(context).requestFocus(_focus);
          } else {
            if (widget.onAccept != null) {
              var args = InputPriceArgs(Convert.toDouble(inputValue));
        ...
        children: <Widget>[
          Expanded(
            child: Container(
              ...
              child: Text("请输入售价", style: TextStyles.getTextStyle(color: Constants.hexStringToColor("#FFFFFF"), fontSize: 32)),
            ),
          ),
          InkWell(
            onTap: () {
