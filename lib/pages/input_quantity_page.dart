...
class InputQuantityPage extends StatefulWidget {
  ...
  InputQuantityPage(this.product, {this.onAccept, this.onClose});

  @override
  _InputQuantityPageState createState() => _InputQuantityPageState();
}

class _InputQuantityPageState extends State<InputQuantityPage> with SingleTickerProviderStateMixin {
  //输入
        ...
        onFieldSubmitted: (inputValue) async {
          if (StringUtils.isBlank(inputValue)) {
            ...
            if (widget.onAccept != null) {
              var args = InputQuantityArgs(Convert.toDouble(inputValue));
        ...
        children: <Widget>[
          Expanded(
            child: Container(
              ...
              child: Text("请输入数量", style: TextStyles.getTextStyle(color: Constants.hexStringToColor("#FFFFFF"), fontSize: 32)),
            ),
          ),
          InkWell(
            onTap: () {
